...
/*
 * JOB ERROR EVENT. Used to indicate an error when attempting to process
 * a job-related command (excluding job submission). The jobId argument
 * is the ID of the job to which the command applies.
 */
proxy_msg *
proxy_job_error_event(int trans_id, char *jobId, int code, char *msg)
{
	proxy_msg *	m = new_proxy_msg(PROXY_EV_ERROR, trans_id);
	
	proxy_msg_add_int(m, 3); /* 3 attributes */
	proxy_msg_add_keyval_string(m, ELEMENT_ID_ATTR, jobId);
	proxy_msg_add_keyval_int(m, ERROR_CODE_ATTR, code);
	proxy_msg_add_keyval_string(m, ERROR_MSG_ATTR, msg);

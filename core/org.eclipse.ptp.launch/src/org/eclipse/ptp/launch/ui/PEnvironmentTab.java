 ...
 *
 */
public class PEnvironmentTab extends PLaunchConfigurationTab {
	...
	class EntryDialog extends Dialog {
		...
		public EntryDialog(String name, String value, boolean edit) {
			super(PEnvironmentTab.this.getControl().getShell());
			fName = name;
			fValue = value;
			fEdit = edit;
		}
		...
		protected void configureShell(Shell shell) {
			...
			String title = (fEdit) ? LaunchMessages.getResourceString("PEnvironmentTab.Edit_Variable") : LaunchMessages.getResourceString("PEnvironmentTab.New_Variable"); //$NON-NLS-1$ //$NON-NLS-2$
			shell.setText(title);
		}
		...
		protected Control createDialogArea(Composite parent) {
			...
			label.setText(LaunchMessages.getResourceString("PEnvironmentTab.NameColon")); //$NON-NLS-1$
			...
			label.setText(LaunchMessages.getResourceString("PEnvironmentTab.ValueColon")); //$NON-NLS-1$
	...
	private void createVariableList(Composite parent) {
		...
		column1.setText(LaunchMessages.getResourceString("PEnvironmentTab.Name")); //$NON-NLS-1$
		tableLayout.addColumnData(new ColumnWeightData(30));

		TableColumn column2 = new TableColumn(table, SWT.NULL);
		column2.setText(LaunchMessages.getResourceString("PEnvironmentTab.Value")); //$NON-NLS-1$
		tableLayout.addColumnData(new ColumnWeightData(30));
	...
	private void createButtons(Composite parent) {
		...
		fBtnNew.setText(LaunchMessages.getResourceString("PEnvironmentTab.New...")); //$NON-NLS-1$
		...
		fBtnImport.setText(LaunchMessages.getResourceString("PEnvironmentTab.Import...")); //$NON-NLS-1$
		...
		fBtnEdit.setText(LaunchMessages.getResourceString("PEnvironmentTab.Edit...")); //$NON-NLS-1$
		...
		fBtnRemove.setText(LaunchMessages.getResourceString("PEnvironmentTab.Remove")); //$NON-NLS-1$
		...
		try {
			...
			while((line = reader.readLine()) != null) {
				...
				if(fElements.getProperty(key) != null) {
					boolean overwrite;
					overwrite = MessageDialog.openQuestion(getShell(), LaunchMessages.getResourceString("PEnvironmentTab.Existing_Environment_Variable"), LaunchMessages.getFormattedResourceString("PEnvironmentTab.Environment_variable_NAME_exists", key)); //$NON-NLS-1$ //$NON-NLS-2$
					if(!overwrite) {
						continue;
					}
				}
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy config) {
		config.setAttribute(IPTPLaunchConfigurationConstants.ATTR_PROGRAM_ENVIROMENT_MAP, (Map) null);
		config.setAttribute(IPTPLaunchConfigurationConstants.ATTR_PROGRAM_ENVIROMENT_INHERIT, true);
	}

	public void initializeFrom(ILaunchConfiguration config) {
		try {
			Map env = config.getAttribute(IPTPLaunchConfigurationConstants.ATTR_PROGRAM_ENVIROMENT_MAP, (Map) null);
			if (env != null) {
				fElements.clear();
	...
	public void performApply(ILaunchConfigurationWorkingCopy config) {
		config.setAttribute(IPTPLaunchConfigurationConstants.ATTR_PROGRAM_ENVIROMENT_MAP, (Map) fElements.clone());
		config.setAttribute(IPTPLaunchConfigurationConstants.ATTR_PROGRAM_ENVIROMENT_INHERIT, true);
	}
	...
	public String getName() {
		return LaunchMessages.getResourceString("PEnvironmentTab.Environment"); //$NON-NLS-1$
	}
	...
	public Image getImage() {
		return LaunchImages.getImage(LaunchImages.IMG_ENVIRONMENT_TAB);
	}

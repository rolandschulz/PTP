	...
	public IProxyEvent toEvent(ProxyPacket packet) {
		...
		case IProxyDebugEvent.EVENT_DBG_SIGNALS:
			...
			for (int i = 0; i < numSignals; i++) {
				signals[i] = new ProxyDebugSignal(args[5 * i + 2], toBoolean(Integer.parseInt(args[5 * i + 3])),
						toBoolean(Integer.parseInt(args[5 * i + 4])), toBoolean(Integer.parseInt(args[5 * i + 5])), args[5 * i + 6]);
			}
		...
		case IProxyDebugEvent.EVENT_DBG_DATA_READ_MEMORY:
			...
			evt = new ProxyDebugMemoryInfoEvent(packet.getTransID(), bits, toMemoryInfo(args[1], args[2], args[3], args[4],
					args[5], args[6], args[7], memories));
			break;

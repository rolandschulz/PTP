...
public class PTPTargetControl extends SSHTargetControl implements ITargetVariables {
	 ...
	 */
	private final ITargetElement targetElement;
	...
	/**
	 * Creates a target control. If some attribute related to the environment
	 * has an invalid format, an exception is thrown. Simulator attributes are
	 * not checked yet, but will be checked when the target is created
	 * (launched).
	 * 
	 * @param attributes
	...
	/**
	 * Connect to the remote host. On every error or possible failure, an
	...
	public boolean create(IProgressMonitor monitor) throws CoreException {
		...
		if (currentTargetConfig.isPasswordAuth()) {
			setConnectionParameters(new SSHParameters(currentTargetConfig.getConnectionAddress(),
					currentTargetConfig.getConnectionPort(), currentTargetConfig.getLoginUserName(),
					currentTargetConfig.getLoginPassword(), currentTargetConfig.getCipherType(),
					currentTargetConfig.getConnectionTimeout() * 1000));
		} else {
			setConnectionParameters(new SSHParameters(currentTargetConfig.getConnectionAddress(),
					currentTargetConfig.getConnectionPort(), currentTargetConfig.getLoginUserName(),
					currentTargetConfig.getKeyPath(), currentTargetConfig.getKeyPassphrase(), currentTargetConfig.getCipherType(),
					currentTargetConfig.getConnectionTimeout() * 1000));
		}
		...
		try {
			...
			disconnect();
			setState(NOT_OPERATIONAL);
			throw new CoreException(new Status(IStatus.ERROR, RemoteToolsAdapterCorePlugin.getUniqueIdentifier(), e.getMessage()));
		}
		monitor.done();
		return true;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ptp.remotetools.environment.extension.ITargetVariables#
	 * getSystemWorkspace()
	 */
	...
	public boolean resume(IProgressMonitor monitor) throws CoreException {
		throw new CoreException(new Status(IStatus.ERROR, getPluginId(), 0, Messages.TargetControl_resume_CannotResume, null));
	}
	...
	public boolean stop(IProgressMonitor monitor) throws CoreException {
		throw new CoreException(new Status(IStatus.ERROR, getPluginId(), 0, Messages.TargetControl_stop_CannotPause, null));
	}
	 ...
	 * 
	 * @see
	 * org.eclipse.ptp.remotetools.environment.control.SSHTargetControl#getPluginId
	 * ()
	 */
	@Override
	protected String getPluginId() {
		return RemoteToolsAdapterCorePlugin.getUniqueIdentifier();
	}
}

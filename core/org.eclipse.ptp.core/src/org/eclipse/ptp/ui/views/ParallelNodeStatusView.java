...
public class ParallelNodeStatusView extends AbstractParallelView implements IParallelLaunchListener, ISelectionProvider
{
    ...
    public final static int SELECTED = 1;
   	...
   	/* these UI components are global so that different methods can
   	 * force updates on them and change their contents
   	 */
    ...
    private MISystemDescription sysDescNodes[];
    ...
    /* these globals pertain to how the canvas is drawn, such as offsets
     * and spacings between nodes.
     */
	...
	final int y_offset = 10;
   	...
   	/* the node, if any, that the user has selected with their
   	 * mouse - we draw a box around that node 
   	 */
    ...
    /* called to refresh the GUI - this asyncExec is needed because
     * there are many threads running around in there calling different
     * things and the event threads cannot access GUI controls, this
     * allows us to fire off an update of the GUI async
     */
    public void refresh(final boolean resize, final IPElement el) {
        Display.getDefault().asyncExec(new Runnable() {
            public void run() {
            	...
            	if(resize) computeNewCanvasSize();
            	
            	if(el == null) {
            		//System.out.println("___ WARNING: FULL SCREEN REDRAW!!! ____");
            		c.redraw();
            	}
            		...
            		if(index >= 0) {
            			...
            			x = index % visible_node_cols;
               	  		...
               	  		y_loc = y_offset + y_spacing * ((index / visible_node_cols) + 1) + 
    ...
    private void filterNodes(boolean user_changed_filter)
    {
    	...
    	if(filterSelection != ALL_NODES) {
    		MISystemDescription[] tmpNodes = new MISystemDescription[sysDescNodes.length];
    		int copied_elements = 0;
    		/* we're going to go through the sysDescNodes array and choose to copy elements
    			...
    			if(filterSelection == USED_NODES) {
    				...
    				if(somenode != null && somenode.hasChildren()) {
    					...
    					if(procs != null && procs.length > 0) {
    						tmpNodes[copied_elements] = sysDescNodes[i];
    						copied_elements++;
    					}
    				}
    			}
    		...
    		System.out.println("FILTER: of "+sysDescNodes.length+" nodes I filtered down to "+copied_elements);
    		/* reallocate that array and then copy our elements into it */
    		sysDescNodes = new MISystemDescription[copied_elements];
    		System.arraycopy(tmpNodes, 0, sysDescNodes, 0, copied_elements);
    	}
    ...
    /* fill the array sysDescNodes[] with the status of the system at
     * current time 
     */
    private void grabSystemStatus()
    {
        try {
            ...
            sysDescNodes = sysStatus.getMISysStatusInfo().getMISystemDescription();            
        } catch (MIException e) {
        }
    }
    ...
    private void getInitialStatus()
    {
        ...
        session = launchManager.getSession();
        if (session == null) return;
    ...
    public void refreshOneProcStatus(PProcess proc) {
    	/*
    	System.out.println("old status = "+processList[proc.getKeyNumber()].getStatus());
    	processList[proc.getKeyNumber()] = proc;
    	System.out.println("new status = "+processList[proc.getKeyNumber()].getStatus());
    	*/
    	refresh(true, proc);
    }
    ...
    private void fillUpProcessArray()
    {
    	...
    	for(int i=0; i<launchedNodes.length; i++) {
    		if(launchedNodes[i].hasChildren())
    			num_procs += launchedNodes[i].getProcesses().length;
    	}
    	processList = new IPProcess[num_procs];

    	for(int i=0; i<launchedNodes.length; i++) {
    		if(launchedNodes[i].hasChildren()) {
    			IPProcess tmp[] = getSortedProcessList(launchedNodes[i]);
    			
    			for(int j=0; j<tmp.length; j++) {
    				processList[tmp[j].getKeyNumber()] = (IPProcess) tmp[j];
    			}
    		}
    	}
    }
	...
	protected void createControl(Composite parent) {        
	    ...
	    /* makes it so that our scrollbar steps in sizes approximately equal
	     * to a node's heightf
	     */
	    ...
	    /* this is kind of crazy - i need two types of layouts on the bottom region
	     * so I'll nest two composites inside of each other, each with the appropriate
	     * layout
	     */
	    
	    /* outer bottom composite - this one has a form layout to fit nicely with the
	     * canvas above
	     */
	    ...
	    c.addPaintListener(new PaintListener() {
	    	public void paintControl(PaintEvent e) {
	    		//System.out.println("PAINT: x="+e.x+", y="+e.y+", width="+e.width+", height="+e.height);
	    		paintCanvas(e.gc);
	    	}
	    });
	    ...
	    Listener mouseListener = new Listener() { 
	    	   public void handleEvent(Event e) {
	    	   	 /* only bother sending it to the bounds tester if they released the
	    	   	  * mouse - signifying a click
	    	   	  */
	    	   	 if(e.type == SWT.MouseDown || e.type == SWT.MouseDoubleClick || e.type == SWT.MouseMove) {
	    	   	 	...
	    	   	 	if(ob != null) {
							...
							if(bool.booleanValue() == true) {
								/* true means they showed a tool tip - we don't have to do anything */
							}
							else {
								hideToolTip();
							}
    ...
    public void searchForNode(int node)
    {
    	...
    	if(sysDescNodes == null) return;
    	
    	if(mode == PROCESSES) {
    		
    		IPNode somenode = systemNodeToIPNode(""+node+"");
    		if(somenode == null || !somenode.hasChildren()) {
    			MessageBox mb = new MessageBox(this.getViewSite().getShell(), SWT.ICON_ERROR | SWT.OK);
     ...
     * so that it properly shows the selected element - it requires setting
     * the scrollbar value and also causing the content inside of the 
    	...
    	if(procNode < 0) {
    		...
    		for(int i=0; i<procs.length; i++) {
				...
				try {
					...
					if(procInt.intValue() == process) which = i;
    ...
    public int findNodeIndexNumber(int node_num) {
    	...
    	for(int i=0; i<sysDescNodes.length; i++) {
    		...
    		try {
    			Integer nodeInt = new Integer(nodeString);
    ...
    private int getElementImageIndex(int index)
    {
    	if(mode == NODES) {
        	...
        	if(somenode != null && somenode.hasChildren()) {
        		...
        		if(procs != null && procs.length > 0) {
    	  			boolean all_running = true;
       	  			/* run through all the procs and make sure they are all running */
       	  			for(int p=0; p<procs.length; p++) {
       	  				/* if any aren't running - then we'll just display it as red */
        	...
        	if(processList == null || index < 0 || index >= processList.length)
        		return PROC_ERROR;
        	if(processList[index] == null) return PROC_ERROR;
        	
        	String status = processList[index].getStatus();
        	if(status == null) return PROC_ERROR;
    ...
    private void computeNewCanvasSize()
    {
    	...
    	if(mode == PROCESSES && processList != null) num_elements = processList.length;
    	...
    	newHeight = (y_offset * 2) + (num_elements / howManyWide) * (y_spacing + node_height);
    	
    	/* the above eq doesn't account for the last line if it would result in a 
    	 * partially full (in the width) line - this next line adds that extra
    	 * line on
    	 */
	...
	/* called by the mouse listener and tests if the x,y location is within the bounds
	 * of any of our nodes (boxes).  If so, then it display information where is
	 * appropriate based on the mouse event
	 * returns 'true' if it was within the bounds
	 */
	private Object testBounds(int x, int y, int mouseEvent) {
		...
		int num_elements;
		if(mode == PROCESSES && processList != null) num_elements = processList.length;
		else num_elements = num_nodes;
		
		if(x_test >= 0 && x_test < x_spacing + node_width && (x - x_offset) < (node_width * visible_node_cols + x_spacing * visible_node_cols)) {
		...
		if(y_test >= 0 && y_test < y_spacing + node_height) {
			boolean fail = false;
			/* if we're past the last full line of nodes, perhaps we're over the edge? */
			if ((y - y_offset) > (node_height * (num_elements / visible_node_cols) + y_spacing * (num_elements / visible_node_cols))) {
				if(x_box + num_elements - num_elements % visible_node_cols >= num_elements) {
					/* too far! */
					y_box = -1;
					fail = true;
				}
				if((y - y_offset) > (node_height * ((num_elements / visible_node_cols) + 1) + y_spacing * ((num_elements / visible_node_cols) + 1))) {
					y_box = -1;
					fail = true;
				}
			}
			if(!fail)
				y_box = (y - y_offset - y_spacing / 2) / (y_spacing + node_height);
		}
		...
		if(x_box != -1 && y_box != -1) {		
			...
			if(mouseEvent == SWT.MouseDoubleClick && mode == PROCESSES) {
				...
				/* returning true here will keep the tooltip visible but won't redraw everything */
				if(node_num == last_tooltip_node) return new Boolean(true);
			...
			if(show_tool_tip) {
				...
				tipy = y_offset + y_box * (y_spacing + node_height) + node_height + 18;
	 	...
	 	public void run() {
	 		...
	 		Display.getDefault().asyncExec(new Runnable() {
	 			public void run() {
	 				...
	 				if(map.x < 0 || map.y < 0 || map.x > size.x || map.y > size.y) {
	 					hideToolTip();
	 				}
	...
	/* this is taken for the most part from CTabFolder which allows us to 
	 * 'fake' ToolTips
	 */
	...
	private void showToolTip (int x, int y, String str) {
		...
		if(toolTipShell == null) {
			toolTipShell = new Shell(this.getViewSite().getShell(), SWT.ON_TOP | SWT.TOOL);
		}
	...
	{	
		...
		if(mode == NODES) {
			...
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"State",sysDescNodes[selected_node_num].getBprocState()});
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"User",sysDescNodes[selected_node_num].getBprocUser()});
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"Group",sysDescNodes[selected_node_num].getBprocGroup()});
			item = new TableItem(BLtable, 0);
			...
			if(somenode != null && somenode.hasChildren()) {
				...
				for(int i=0; i<procs.length; i++) {
					...
					else if(procs[i].getStatus().equals(IPProcess.EXITED_SIGNALLED)) {
						item.setImage(0, statusImages[PROC_EXITED_SIGNAL][NOT_SELECTED]);
					}
			...
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"Rank",processList[selected_node_num].getProcessNumber()});
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"PID",processList[selected_node_num].getPid()});
			...
			item = new TableItem(BLtable, 0);
			item.setText(new String[]{"Status",processList[selected_node_num].getStatus()});
			if(processList[selected_node_num].getStatus().equals(IPProcess.EXITED)) {
				item = new TableItem(BLtable, 0);
				item.setText(new String[]{"Exit code",processList[selected_node_num].getExitCode()});
			}
			else if(processList[selected_node_num].getStatus().equals(IPProcess.EXITED_SIGNALLED)) {
				item = new TableItem(BLtable, 0);
				item.setText(new String[]{"Signal name",processList[selected_node_num].getSignalName()});
			}
		
			item = new TableItem(BRtable, 0);
			
			if(processList[selected_node_num].getStatus().equals(IPProcess.STARTING))
				item.setImage(0, statusImages[PROC_STARTING][NOT_SELECTED]);
			else if(processList[selected_node_num].getStatus().equals(IPProcess.RUNNING))
				item.setImage(0, statusImages[PROC_RUNNING][NOT_SELECTED]);
			else if(processList[selected_node_num].getStatus().equals(IPProcess.EXITED))
				item.setImage(0, statusImages[PROC_EXITED][NOT_SELECTED]);
			else if(processList[selected_node_num].getStatus().equals(IPProcess.EXITED_SIGNALLED))
				item.setImage(0, statusImages[PROC_EXITED_SIGNAL][NOT_SELECTED]);
			else if(processList[selected_node_num].getStatus().equals(IPProcess.STOPPED))
				item.setImage(0, statusImages[PROC_STOPPED][NOT_SELECTED]);
			else if(processList[selected_node_num].getStatus().equals(IPProcess.ERROR))
				item.setImage(0, statusImages[PROC_ERROR][NOT_SELECTED]);
			item.setText(1,"Process "+processList[selected_node_num].getProcessNumber());		
		}
		
		/* we want to reselect / rehighlight any selection they might
		 * have previously made before we redrew this area 
		 */
		if(selected_node_num == last_node_num && idx >= 0 && idx < BRtable.getItemCount())
     ...
     * @see org.eclipse.pdt.ui.views.ISelectedElement#selectedElement()
     */
    ...
    public ISelection getSelection() {
		...
		if(element == null) return StructuredSelection.EMPTY;

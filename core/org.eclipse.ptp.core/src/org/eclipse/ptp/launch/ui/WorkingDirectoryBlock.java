...
public class WorkingDirectoryBlock extends PLaunchConfigurationTab {
    ...
    public void createControl(Composite parent) {
		...
		workingDirLabel.setText(UIMessage.getResourceString("WorkingDirectoryBlock.Working_directory_colon"));
		workingDirLabel.setLayoutData(spanGridData(-1, 3));

		useDefaultWorkingDirButton = new Button(workingDirComp, SWT.CHECK);
		useDefaultWorkingDirButton.setText(UIMessage.getResourceString("WorkingDirectoryBlock.Use_de&fault_working_directory"));
		useDefaultWorkingDirButton.setLayoutData(spanGridData(-1, 3));
		useDefaultWorkingDirButton.addSelectionListener(listener);
		
		localDirButton = createRadioButton(workingDirComp, UIMessage.getResourceString("WorkingDirectoryBlock.&Local_directory"));
		...
		workingDirBrowseButton = createPushButton(workingDirComp, UIMessage.getResourceString("Tab.common.&Browse_1"), null);
		workingDirBrowseButton.addSelectionListener(listener);
		
		workspaceDirButton = createRadioButton(workingDirComp, UIMessage.getResourceString("WorkingDirectoryBlock.Works&pace"));
		...
		workspaceDirBrowseButton = createPushButton(workingDirComp, UIMessage.getResourceString("Tab.common.B&rowse_2"), null);
	...
	public boolean isValid(ILaunchConfiguration configuration) {
		...
		if (isLocalWorkingDirectory()) {		    
			...
			if (workingDirPath != null) {
				...
				if (!dir.exists()) {
					setErrorMessage(UIMessage.getResourceString("WorkingDirectoryBlock.Working_directory_does_not_exist"));
					return false;
				}
				if (!dir.isDirectory()) {
					setErrorMessage(UIMessage.getResourceString("WorkingDirectoryBlock.Working_directory_is_not_a_directory"));
					return false;
				}
			}
		} else {
			if (getContainer(workingDirText.getText()) == null) {
				setErrorMessage(UIMessage.getResourceString("WorkingDirectoryBlock.Project_or_folder_does_not_exist"));
				return false;
			}
		}		
		return true;
    ...
    public String getName() {
        return UIMessage.getResourceString("WorkingDirectoryBlock.Working_directory");
    }
	...
	protected void handleWorkingDirBrowseButtonSelected() {
		...
		dialog.setMessage(UIMessage.getResourceString("WorkingDirectoryBlock.Select_&working_directory_for_launch_configuration"));
	...
	protected void handleWorkspaceDirBrowseButtonSelected() {
		ContainerSelectionDialog dialog = new ContainerSelectionDialog(getShell(), getWorkspaceRoot(), false, UIMessage.getResourceString("WorkingDirectoryBlock.Select_&workspace_relative_working_directory"));		

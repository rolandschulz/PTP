    ...
    private IPerspectiveListener perspectiveListener = new IPerspectiveListener() {
        public void perspectiveClosed(IWorkbenchPage page, IPerspectiveDescriptor perspective) {
            if (perspective.getId().equals(CoreUtils.PPerspectiveFactory_ID)) {
    			...
    			IWorkbench workbench = PTPCorePlugin.getDefault().getWorkbench();    			
 ...
 /*
    ...
    */
    ...
    protected String renderLabel(String name) {
        String format = CoreMessages.getResourceString("LaunchManager.{0}_({1})");
    ...
    protected void isSessionExist() throws CoreException {
        ...
        if (session == null) {
            createMPISession();
            //Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, "No MI session is created", null);
    ...
    public synchronized void mpisysstatus() throws CoreException {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, "Cannot exec SYS STATUS command", e);
            throw new CoreException(status);
        }
        */
    }
    ...
    public synchronized void mpistatus() throws CoreException {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, "Cannot exec STATUS command", e);
            throw new CoreException(status);
        }
        */
    }
    ...
    public synchronized void mpiabort() throws CoreException {
        ...
        try {
            ...
            Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, "Cannot exec ABORT command", e);
            throw new CoreException(status);
        }
        */
    }
    ...
    public synchronized void mpiexit() throws CoreException {
        ...
        try {
            ...
            */
            ...
            Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, "Cannot exec EXIT command", e);
            throw new CoreException(status);
        } finally {
            clearAll();            
        }
    ...
    public synchronized void createMPISession() throws CoreException {
    	/*
        if (session == null) {
            MIPlugin miPlugin = MIPlugin.getDefault(PTPCorePlugin.getDefault().getPluginPreferences());
            ...
            try {
                ...
                Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, CoreMessages.getResourceString("LaunchManager.Exception_occurred_executing_command_line"), e);
                throw new CoreException(status);
            } catch (IOException e) {
                Status status = new Status(IStatus.ERROR, PTPCorePlugin.getUniqueIdentifier(), IStatus.INFO, CoreMessages.getResourceString("LaunchManager.Exception_occurred_executing_command_line"), e);
                throw new CoreException(status);

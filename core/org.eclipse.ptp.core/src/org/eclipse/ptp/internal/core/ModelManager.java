...
public class ModelManager implements IModelManager, IRuntimeListener {
	...
	*/
		...
		for (int i = 0; i < ne.length; i++) {
			...
			if(monitoringSystem instanceof OMPIMonitoringSystem) {
				...
				for (int j = 0; j < ne2.length; j++) {
					...
					node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_NAME, nodename);
					System.out.println("NodeName According to ORTE = '"+node.getAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_NAME)+"'");
					System.out.println("\t#attribs returned: "+attribs.length);
					
					if(attribs.length > (j * num_attribs) + 1) {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_USER,
								attribs[(j * num_attribs) + 1]);
					}
					else {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_USER, "UNKNOWN");
					}
					
					if(attribs.length > (j * num_attribs) + 2) {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_GROUP,
								attribs[(j * num_attribs) + 2]);
					}
					else {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_GROUP, "UNKNOWN");
					}
					
					if(attribs.length > (j * num_attribs) + 3) {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_STATE,
							attribs[(j * num_attribs) + 3]);
					}
					else {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_STATE, "up");
					}
					
					if(attribs.length > (j * num_attribs) + 4) {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_MODE,
							attribs[(j * num_attribs) + 4]);
					}
					else {
						node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_MODE, "73");
					}					
					mac.addChild(node);
					subMonitor.worked(1);
				}
				...
				for(int j=0; j<ne2.length; j++) {
					...
					PNode node = new PNode(mac, ne2[j], ""+j+"", j);
					node.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_NAME, 
	...
	public void runtimeNodeGeneralChange(String ne, String key, String value) {
		...
		if(n != null) {
			System.out.print("\t before, val = "+n.getAttribute(AttributeConstants.ATTRIB_CLASS_NODE, key));
			n.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, key, value);
			System.out.println("\t after, val = "+n.getAttribute(AttributeConstants.ATTRIB_CLASS_NODE, key));
	...
	public void runtimeNodeStatusChange(String ne) {
		...
		if (n != null) {
			try {
				n.setAttribute(AttributeConstants.ATTRIB_CLASS_NODE, AttributeConstants.ATTRIB_NODE_STATE, monitoringSystem.getNodeAttributes(n, new String[] {AttributeConstants.ATTRIB_NODE_STATE}));
				fireEvent(n, EVENT_SYS_STATUS_CHANGE);
			} catch(CoreException e) {

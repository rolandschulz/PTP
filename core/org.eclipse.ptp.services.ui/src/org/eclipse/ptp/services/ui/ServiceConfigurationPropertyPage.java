		...
		IWorkbenchPropertyPage 
{
	...
	protected Control createContents(Composite parent) 
	{
		...
		parent.setLayout(parentPaneLayout);
			...
			// Create the properties pane, which will contain the service configuration list
		...
		serviceModelControl = serviceModelWidget.createContents(serviceModelPane);
		...
		widgetPane = propertiesPane;
			// Fill in the list of service configurations currently used by this project
		getProjectConfigurations();
		return propertiesPane;
	}
	
	/**
	 * Get the list of service configuration used by this project, sort by name and then
	 * populate the service configuration list.
	 */
	private void getProjectConfigurations()
	{
		...
		try {
			serviceConfigurations = ServiceModelManager.getInstance().getConfigurations(getProject()).toArray();
	...
	/**
	 * Add a new service configuration to the list of service configurations used by this
	 * project
	 */
	private void addServiceConfiguration()
	{
		...
		int status;
			...
			// Display a dialog containing a list of available service configurations
		dialog = new ServiceConfigurationSelectionDialog(getShell(), 
						ServiceModelManager.getInstance().getConfigurations(getProject()));
		status = dialog.open();
		if (status == Window.OK) {
			...
			if (config != null) {
				...
				ServiceModelManager.getInstance().addConfiguration(getProject(), config);
			}
		}
	}
	...
	private void removeServiceConfiguration()
	{
		...
		if (selection.length > 0) {
			...
			ServiceModelManager.getInstance().removeConfiguration(getProject(), selectedConfig);

 ...
 */
public class CMemoryBlockExtension extends PDebugElement implements IMemoryBlockExtension, ICDIEventListener {
	 ...
	 */
	public CMemoryBlockExtension( PDebugTarget target, String expression, BigInteger baseAddress ) {
	 ...
	 * Constructor for CMemoryBlockExtension. 
	 */
	public CMemoryBlockExtension( PDebugTarget target, String expression, BigInteger baseAddress, int wordSize ) {
		super( target );
	...
	private ICDIMemoryBlock createCDIBlock( BigInteger address, long length, int wordSize ) throws CDIException {
		ICDIMemoryBlock block = ((PDebugTarget)getDebugTarget()).getCDITarget().createMemoryBlock( address.toString(), (int)length, wordSize );
	...
	private void disposeCDIBlock() {
		...
		if ( block != null ) {
			try {
				((PDebugTarget)getDebugTarget()).getCDITarget().removeBlocks( new ICDIMemoryBlock[]{ block  });
			}
			catch( CDIException e ) {
				DebugPlugin.log( e );
			}
	...
	public void dispose() {
		...
		if ( cdiBlock != null ) {
			try {
				((PDebugTarget)getDebugTarget()).getCDITarget().removeBlocks( new ICDIMemoryBlock[] {cdiBlock} );
			}
			catch( CDIException e ) {

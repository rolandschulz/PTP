 ...
 */
public class CRegisterGroup extends PDebugElement implements IPersistableRegisterGroup, IEnableDisableTarget {
	 ...
	 */
	public CRegisterGroup( PDebugTarget target ) {
		super( target );
	}
	 ...
	 * Constructor for CRegisterGroup.
	 */
	public CRegisterGroup( PDebugTarget target, String name, IRegisterDescriptor[] descriptors ) {
		super( target );
		fName = name;
				...
				if ( ELEMENT_REGISTER.equals( child.getNodeName() ) ) {
					...
					if ( name == null || name.length() == 0 || originalGroupName == null || originalGroupName.length() == 0 ) {
						...
						if ( d != null )
							...
							PTPDebugCorePlugin.log( CoreModelMessages.getString( "CRegisterGroup.4" ) ); //$NON-NLS-1$
					}
				}
			}
	...
	private PRegisterManager getRegisterManager() {
		return (PRegisterManager)getDebugTarget().getAdapter( PRegisterManager.class );
	}

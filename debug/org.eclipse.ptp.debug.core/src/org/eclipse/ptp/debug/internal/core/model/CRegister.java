...
public class CRegister extends CGlobalVariable implements IRegister {
	...
	protected CRegister( CRegisterGroup parent, IRegisterDescriptor descriptor ) {
		...
		setFormat( CVariableFormat.getFormat( PTPDebugCorePlugin.getDefault().getPluginPreferences().getInt( IPDebugConstants.PREF_DEFAULT_REGISTER_FORMAT ) ) );
	}
	...
	protected boolean isBookkeepingEnabled() {
		...
		try {
			result = getLaunch().getLaunchConfiguration().getAttribute( IPTPLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, false );
		}

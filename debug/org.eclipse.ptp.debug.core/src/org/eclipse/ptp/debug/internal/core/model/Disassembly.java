 ...
 */
public class Disassembly extends PDebugElement implements IDisassembly {
	 ...
	 * @param target
	 */
	public Disassembly( PDebugTarget target ) {
		super( target );
	}
	...
	private DisassemblyBlock createBlock( ICStackFrame frame ) throws DebugException {
		IPCDITarget target = (IPCDITarget)getDebugTarget().getAdapter( IPCDITarget.class );
		if ( target != null ) {
			...
			if ( fileName != null && fileName.length() > 0 ) {
				try {
					mixedInstrs = target.getMixedInstructions( fileName, 
							lineNumber, 
							PTPDebugCorePlugin.getDefault().getPluginPreferences().getInt( IPDebugConstants.PREF_MAX_NUMBER_OF_INSTRUCTIONS ) );
				}

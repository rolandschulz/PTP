...
public class CExpression extends CVariable implements IExpression {
	...
	public CExpression( CStackFrame frame, ICDIExpression cdiExpression, ICDIVariableDescriptor varObject ) {
		...
		setFormat( CVariableFormat.getFormat( PTPDebugCorePlugin.getDefault().getPluginPreferences().getInt( IPDebugConstants.PREF_DEFAULT_EXPRESSION_FORMAT ) ) );
	...
	public void handleDebugEvents( ICDIEvent[] events ) {
		for( int i = 0; i < events.length; i++ ) {
			...
			if ( event instanceof ICDIResumedEvent ) {
				...
				if ( source != null ) {
					IPCDITarget cdiTarget = (IPCDITarget) source.getTarget();

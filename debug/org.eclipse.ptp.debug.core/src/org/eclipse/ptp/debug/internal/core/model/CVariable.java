...
public class CVariable extends AbstractCVariable implements ICDIEventListener {
	 ...
	 */
	private CVariableFormat fFormat = CVariableFormat.getFormat( PTPDebugCorePlugin.getDefault().getPluginPreferences().getInt( ICDebugConstants.PREF_DEFAULT_VARIABLE_FORMAT ) );
	...
	public void handleDebugEvents( ICDIEvent[] events ) {
		...
		for( int i = 0; i < events.length; i++ ) {
			...
			if ( source == null )
				continue;
			IPCDITarget target = (IPCDITarget) source.getTarget();

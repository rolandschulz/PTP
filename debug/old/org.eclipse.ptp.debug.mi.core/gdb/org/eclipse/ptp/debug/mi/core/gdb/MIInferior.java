...
import org.eclipse.ptp.debug.mi.core.gdb.command.MIExecAbort;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIExecInterrupt;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIGDBShowExitCode;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIInfoProgram;
...
import org.eclipse.ptp.debug.mi.core.gdb.output.MIInfoProgramInfo;
...
public class MIInferior extends Process {
		...
		if (out == null) {
			out = new OutputStream() {
				...
				public void write(int b) throws IOException {
					if (!isRunning()) {
						throw new IOException("target is suspended"); //$NON-NLS-1$
					}
					OutputStream channel = session.getChannelOutputStream();
					if (channel == null) {
						throw new IOException("No MI Session"); //$NON-NLS-1$
					}
					channel.write(b);
				}
			};
	...
	public void interrupt() throws MIException {
		...
		if (state == RUNNING) {
			throw new MIException("Failed to interrupt"); //$NON-NLS-1$
		}
	}

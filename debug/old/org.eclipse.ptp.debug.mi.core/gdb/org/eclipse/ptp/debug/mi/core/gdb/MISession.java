...
import org.eclipse.ptp.debug.mi.core.gdb.command.CommandFactory;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIExecInterrupt;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIGDBExit;
import org.eclipse.ptp.debug.mi.core.gdb.command.MIGDBSet;
import org.eclipse.ptp.debug.mi.core.gdb.event.MIEvent;
import org.eclipse.ptp.debug.mi.core.gdb.event.MIGDBExitEvent;
import org.eclipse.ptp.debug.mi.core.gdb.output.MIOutput;
import org.eclipse.ptp.debug.mi.core.gdb.output.MIParser;
...
public class MISession extends Observable {
	...
	public MISession(MIProcess process, IMITTY tty, int timeout, int type, int launchTimeout) throws MIException {
		...
		try {
			...
			if (line == null) {
				line = "Process Terminated"; //$NON-NLS-1$
			}
	...
	public void postCommand(Command cmd, long timeout) throws MIException {
		...
		if (!txThread.isAlive() || !rxThread.isAlive()) {
			throw new MIException("{R,T}xThread terminated"); //$NON-NLS-1$
		}
		...
		if (inferior.isRunning()) {
			...
			if (!(cmd instanceof MIExecInterrupt)) {
				throw new MIException("Target is not suspended"); //$NON-NLS-1$
			}
		}

		if (isTerminated()) {
			throw new MIException("Session terminated"); //$NON-NLS-1$
		}
	...
	public synchronized void postCommand0(Command cmd, long timeout) throws MIException {
		...
		GDBSession.getDefault().debugLog(cmd.toString());
			...
			while (cmd.getMIOutput() == null) {
				try {
					...
					if (cmd.getMIOutput() == null) {
						throw new MIException("Target is not responding (timed out)"); //$NON-NLS-1$
					}

...
import org.eclipse.ptp.debug.mi.core.cdi.SessionObject;
...
public class Target extends SessionObject implements IPCDITarget {
	...
	MISession[] miSessions;
	
	public Target(Session s, MISession[] mis) {
		super(s);
		miSessions = mis;
		fConfiguration = new TargetConfiguration(this);
	}

	public MISession[] getMISessions() {
		return miSessions;
	}
	...
	public Process[] getProcesses() {
		Process[] procs = new Process[miSessions.length];
		for (int i = 0; i < miSessions.length; i++) {
			procs[i] = miSessions[i].getMIInferior();
		}
		return procs;
	}

	public Process getProcess() {
		return miSessions[0].getMIInferior();
	}

...
public class SDMPreferencePage extends AbstractPerferencePage {
	...
	protected void defaultSetting() {
		Preferences preferences = PTPDebugExternalPlugin.getDefault().getPluginPreferences();
		URL url = Platform.find(Platform.getBundle(PTPDebugExternalPlugin.PLUGIN_ID), new Path("/"));
		if (url != null) {
			try {
				File path = new File(Platform.asLocalURL(url).getPath());
				String ipath = path.getAbsolutePath();
				int idx = ipath.indexOf(PTPDebugExternalPlugin.PLUGIN_ID);
	...
	public boolean performOk() {
		storeValues();
		PTPDebugExternalPlugin.getDefault().savePluginPreferences();
		return true;
	}

...
import org.eclipse.ptp.debug.external.gdb.mi.command.CLICommand;
import org.eclipse.ptp.debug.external.gdb.mi.output.MIInfo;
import org.eclipse.ptp.debug.external.gdb.mi.utils.PTY;
...
public class GDBSession {
	...
	//The shared instance.
	private static GDBSession plugin;
    ...
    */
	...
	public MISession createSession(String[] gdb, File program, File cwd, String gdbinit, IMITTY pty) throws IOException, MIException {
		...
		if (pty != null) {
			if (program == null) {
				args = new String[] {"--cd="+cwd.getAbsolutePath(), "--command="+gdbinit, "-q", "-nw", "-tty", pty.getSlaveName(), "-i", "mi1"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
			} else {
				args = new String[] {"--cd="+cwd.getAbsolutePath(), "--command="+gdbinit, "-q", "-nw", "-tty", pty.getSlaveName(), "-i", "mi1", program.getAbsolutePath()}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
			}
		} else {
			if (program == null) {
				args = new String[] {"--cd="+cwd.getAbsolutePath(), "--command="+gdbinit, "-q", "-nw", "-i", "mi1"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
			} else {
				args = new String[] {"--cd="+cwd.getAbsolutePath(), "--command="+gdbinit, "-q", "-nw", "-i", "mi1", program.getAbsolutePath()}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
			}
		}
		...
		if (gdb == null || gdb.length == 0) {
			newArgs = new String[1 + args.length];
			newArgs[0] = GDB;
			
			System.arraycopy(args, 0, newArgs, 1, args.length);
		} else {
			newArgs = new String[gdb.length + args.length];
			
			System.arraycopy(gdb, 0, newArgs, 0, gdb.length);
			System.arraycopy(args, 0, newArgs, gdb.length, args.length);
		}
		
		MIProcess pgdb = new MIProcessAdapter(newArgs);

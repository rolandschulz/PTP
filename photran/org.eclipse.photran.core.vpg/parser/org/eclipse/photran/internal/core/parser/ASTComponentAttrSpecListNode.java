...
public class ASTComponentAttrSpecListNode extends InteriorNode
{
    protected int count = -1;

    ASTComponentAttrSpecListNode(Production production, List<CSTNode> childNodes, List<CSTNode> discardedSymbols)
    {
         super(production);
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTComponentAttrSpecListNode
               && grandparent instanceof ASTComponentAttrSpecListNode
               && ((ASTComponentAttrSpecListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    /**
     * @return the number of ASTComponentAttrSpecListNode nodes in this list
     */
    public int size()
    {
    ...
    ASTComponentAttrSpecListNode recurseToIndex(int listIndex)
    {
        ASTComponentAttrSpecListNode node = this;
        for (int depth = size()-listIndex-1, i = 0; i < depth; i++)
        {
            if (node == null) throw new IllegalArgumentException("Index " + listIndex + " out of bounds (size: " + size() + ")");
            node = (ASTComponentAttrSpecListNode)node.getRecursiveNode();
        }
        return node;
    }
    ...
    {
        visitor.visitASTComponentAttrSpecListNode(this);
    }

    public ASTComponentAttrSpecNode getComponentAttrSpec(int listIndex)
    {
    ...
    private ASTComponentAttrSpecListNode getRecursiveNode()
    {
        if (treeHasBeenModified()) throw new IllegalStateException("Accessor methods cannot be called on the nodes of a CST after it has been modified");

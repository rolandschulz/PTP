...
public class ASTExprListNode extends InteriorNode
{
    protected int count = -1;

    ASTExprListNode(Production production, List<CSTNode> childNodes, List<CSTNode> discardedSymbols)
    {
         super(production);
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTExprListNode
               && grandparent instanceof ASTExprListNode
               && ((ASTExprListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    /**
     * @return the number of ASTExprListNode nodes in this list
     */
    public int size()
    {
    ...
    ASTExprListNode recurseToIndex(int listIndex)
    {
        ASTExprListNode node = this;
        for (int depth = size()-listIndex-1, i = 0; i < depth; i++)
        {
            if (node == null) throw new IllegalArgumentException("Index " + listIndex + " out of bounds (size: " + size() + ")");
            node = (ASTExprListNode)node.getRecursiveNode();
        }
        return node;
    }
    ...
    public ASTExprNode getExpr(int listIndex)
    {
        ...
        if (node.getProduction() == Production.EXPR_LIST_215)
            return (ASTExprNode)node.getChild(0);
        else if (node.getProduction() == Production.EXPR_LIST_216)
            return (ASTExprNode)node.getChild(2);
        else
            return null;
    }

    private ASTExprListNode getRecursiveNode()
    {
        if (treeHasBeenModified()) throw new IllegalStateException("Accessor methods cannot be called on the nodes of a CST after it has been modified");

        if (getProduction() == Production.EXPR_LIST_216)
    ...
    public Token getTComma(int listIndex)
    {
        ...
        ASTExprListNode node = recurseToIndex(listIndex);
        if (node.getProduction() == Production.EXPR_LIST_216)

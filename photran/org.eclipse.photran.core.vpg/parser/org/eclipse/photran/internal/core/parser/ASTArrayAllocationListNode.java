...
public class ASTArrayAllocationListNode extends InteriorNode
{
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTArrayAllocationListNode
               && grandparent instanceof ASTArrayAllocationListNode
               && ((ASTArrayAllocationListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    public ASTArrayAllocationNode getArrayAllocation(int listIndex)
    {
        ...
        if (node.getProduction() == Production.ARRAY_ALLOCATION_LIST_345)
            return (ASTArrayAllocationNode)node.getChild(0);
        else if (node.getProduction() == Production.ARRAY_ALLOCATION_LIST_346)
    ...
    private ASTArrayAllocationListNode getRecursiveNode()
    {
        ...
        if (getProduction() == Production.ARRAY_ALLOCATION_LIST_346)
    ...
    {
        if (getProduction() == Production.ARRAY_ALLOCATION_LIST_346 && index == 1)
            return false;
        else
            return true;
    }

...
public class ASTInputItemListNode extends InteriorNode
{
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTInputItemListNode
               && grandparent instanceof ASTInputItemListNode
               && ((ASTInputItemListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    public IInputItem getInputItem(int listIndex)
    {
        ...
        if (node.getProduction() == Production.INPUT_ITEM_LIST_805)
            return (IInputItem)node.getChild(0);
        else if (node.getProduction() == Production.INPUT_ITEM_LIST_806)
            return (IInputItem)node.getChild(2);
        else
            return null;
    }
    ...
    private ASTInputItemListNode getRecursiveNode()
    {
        ...
        if (getProduction() == Production.INPUT_ITEM_LIST_806)
    ...
    {
        if (getProduction() == Production.INPUT_ITEM_LIST_806 && index == 1)
            return false;
        else
            return true;
    }

...
public class ASTPointerStmtObjectListNode extends InteriorNode
{
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTPointerStmtObjectListNode
               && grandparent instanceof ASTPointerStmtObjectListNode
               && ((ASTPointerStmtObjectListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    public ASTPointerStmtObjectNode getPointerStmtObject(int listIndex)
    {
        ...
        if (node.getProduction() == Production.POINTER_STMT_OBJECT_LIST_351)
            return (ASTPointerStmtObjectNode)node.getChild(0);
        else if (node.getProduction() == Production.POINTER_STMT_OBJECT_LIST_352)
    ...
    private ASTPointerStmtObjectListNode getRecursiveNode()
    {
        ...
        if (getProduction() == Production.POINTER_STMT_OBJECT_LIST_352)
    ...
    {
        if (getProduction() == Production.POINTER_STMT_OBJECT_LIST_352 && index == 1)
            return false;
        else
            return true;
    }

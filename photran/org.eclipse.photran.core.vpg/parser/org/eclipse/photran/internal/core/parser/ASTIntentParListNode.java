...
public class ASTIntentParListNode extends InteriorNode
{
    protected int count = -1;

    ASTIntentParListNode(Production production, List<CSTNode> childNodes, List<CSTNode> discardedSymbols)
    {
         super(production);
    ...
    @Override public InteriorNode getASTParent()
    {
        ...
        while (parent != null && grandparent != null
               && parent instanceof ASTIntentParListNode
               && grandparent instanceof ASTIntentParListNode
               && ((ASTIntentParListNode)grandparent).getRecursiveNode() == parent)
        {
    ...
    /**
     * @return the number of ASTIntentParListNode nodes in this list
     */
    public int size()
    {
    ...
    ASTIntentParListNode recurseToIndex(int listIndex)
    {
        ASTIntentParListNode node = this;
        for (int depth = size()-listIndex-1, i = 0; i < depth; i++)
        {
            if (node == null) throw new IllegalArgumentException("Index " + listIndex + " out of bounds (size: " + size() + ")");
            node = (ASTIntentParListNode)node.getRecursiveNode();
        }
        return node;
    }
    ...
    public ASTIntentParNode getIntentPar(int listIndex)
    {
        ...
        if (node.getProduction() == Production.INTENT_PAR_LIST_316)
            return (ASTIntentParNode)node.getChild(0);
        else if (node.getProduction() == Production.INTENT_PAR_LIST_317)
    ...
    private ASTIntentParListNode getRecursiveNode()
    {
        ...
        if (getProduction() == Production.INTENT_PAR_LIST_317)
    ...
    public Token getTComma(int listIndex)
    {
        ...
        ASTIntentParListNode node = recurseToIndex(listIndex);
        if (node.getProduction() == Production.INTENT_PAR_LIST_317)

package org.eclipse.photran.internal.core.parser;

import org.eclipse.photran.internal.core.lexer.Token;
import org.eclipse.photran.internal.core.parser.Parser.*;
import java.util.List;

public class ASTDataIDoObjectNode extends InteriorNode
{
    ASTDataIDoObjectNode(Production production, List<CSTNode> childNodes, List<CSTNode> discardedSymbols)
    {
         super(production);
         
         for (Object o : childNodes)
             addChild((CSTNode)o);
         constructionFinished();
    }
    
    @Override protected void visitThisNodeUsing(ASTVisitor visitor)
    {
        visitor.visitASTDataIDoObjectNode(this);
    }

    public ASTArrayElementNode getArrayElement()
    {
        if (treeHasBeenModified()) throw new IllegalStateException("Accessor methods cannot be called on the nodes of a CST after it has been modified");

        if (getProduction() == Production.DATA_IDO_OBJECT_381)
            return (ASTArrayElementNode)getChild(0);
        else
            return null;
    }

    public ASTDataImpliedDoNode getDataImpliedDo()
    {
        if (treeHasBeenModified()) throw new IllegalStateException("Accessor methods cannot be called on the nodes of a CST after it has been modified");

        if (getProduction() == Production.DATA_IDO_OBJECT_382)
            return (ASTDataImpliedDoNode)getChild(0);
        else
            return null;
    }

    public ASTStructureComponentNode getStructureComponent()
    {
        if (treeHasBeenModified()) throw new IllegalStateException("Accessor methods cannot be called on the nodes of a CST after it has been modified");

        if (getProduction() == Production.DATA_IDO_OBJECT_383)
            return (ASTStructureComponentNode)getChild(0);
        else
            return null;
    }
}

...
public class TranslationUnit extends Openable implements ITranslationUnit {
	...
	public IFile getFile() {	// JO -- Made public
	...
	private void parse(Map newElements){
		try {
			...
			// If we're not C or C++, find a different way
			IAdditionalLanguage lang = AdditionalLanguagesExtension.getInstance().getLanguageForSourceContentType(fContentTypeID);
			if (lang != null)

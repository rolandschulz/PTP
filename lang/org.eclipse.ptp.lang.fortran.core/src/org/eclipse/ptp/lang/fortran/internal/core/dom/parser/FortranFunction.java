 ...
 * @author aniefer
 */
public class FortranFunction extends PlatformObject implements IFunction, ICInternalFunction {
	...
	public FortranFunction( IASTFunctionDeclarator declarator ){
		if( declarator != null ) {
		...
		if (dtor instanceof IASTStandardFunctionDeclarator) {
			...
			if( names.length > 0 ){
				...
				for( int i=0; i<names.length; i++ ) {
				    IASTDeclarator decl = FortranVisitor.getKnRParameterDeclarator( (ICASTKnRFunctionDeclarator) dtor, names[i] );
				    if( decl != null ) {
				        result[i] = (IParameter) decl.getName().resolveBinding();
				    } else {
				        result[i] = new FortranParameter.CParameterProblem( names[i], IProblemBinding.SEMANTIC_KNR_PARAMETER_DECLARATION_NOT_FOUND, names[i].toCharArray() );
				    }
				}
			}
		}
	...
	public IScope getScope() {
	    IASTFunctionDeclarator dtor = (IASTFunctionDeclarator) getPhysicalNode();
	    if( dtor != null )
	    	return FortranVisitor.getContainingScope( dtor.getParent() );
	    return null;
	}
    ...
    public IFunctionType getType() {
        if( type == null ) {
        	...
        	if( functionDtor != null ) {
	        	...
	        	IType tempType = FortranVisitor.createType( functionDtor );
    ...
    public IBinding resolveParameter( IASTName paramName ){
    	...
    	if( parent instanceof IASTParameterDeclaration ){
        	...
        	knrParamDtor = FortranVisitor.getKnRParameterDeclarator( fKnRDtor, paramName );
    	...
    	binding = new FortranParameter( paramName );
    	...
    	if( definition != null ){
    	    if( definition instanceof IASTStandardFunctionDeclarator ){
    	    	...
    	    	if( parameterNames.length > idx ) {
	    	        ...
	    	        IASTDeclarator dtor = FortranVisitor.getKnRParameterDeclarator( fKnRDtor, n );
	    	        if( dtor != null ){
    ...
    protected void updateParameterBindings( IASTFunctionDeclarator fdtor ){
        ...
        if( fdtor instanceof IASTStandardFunctionDeclarator ){
        	...
        	for( int i = 0; i < nps.length; i++ ){
        		...
        		if( params[i] instanceof FortranParameter )
        			((FortranParameter)params[i]).addDeclaration( name );
        	}
        } else {
            ...
            for( int i = 0; i < params.length; i++ ){
            	IASTName name = ns[i];
            	name.setBinding( params[i] );
            	IASTDeclarator dtor = FortranVisitor.getKnRParameterDeclarator( (ICASTKnRFunctionDeclarator) fdtor, name );
    			if( dtor != null ){
    			    dtor.getName().setBinding( params[i] );
    			    if( params[i] instanceof FortranParameter )
    			    	((FortranParameter)params[i]).addDeclaration( dtor.getName() );
    			}
        	}
        }
    }

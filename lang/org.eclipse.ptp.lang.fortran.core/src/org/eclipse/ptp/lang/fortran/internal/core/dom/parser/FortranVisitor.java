 ...
 * @author aniefer
 */
public class FortranVisitor {
	public static class ClearBindingAction extends ASTVisitor {
		{
...
//						return PROCESS_CONTINUE;
	...
	static protected void createBinding( IASTName name ){
		...
		if( parent instanceof FortranASTIdExpression ){
	...
	private static IBinding createBinding( ICASTEnumerationSpecifier enumeration ){
        ...
        if( binding != null ){
	        if( binding instanceof FortranEnumeration )
	            ((FortranEnumeration)binding).addDefinition( name );
	        else
	            return new ProblemBinding( name, IProblemBinding.SEMANTIC_INVALID_OVERLOAD, name.toCharArray() );
	    } else {
	        binding = new FortranEnumeration( name );
	        try {
	...
	private static IBinding createBinding( IASTEnumerator enumerator ){
	    IEnumerator binding = new FortranEnumerator( enumerator ); 
	    try {
	...
	private static IBinding createBinding( IASTStatement statement ){
	    if( statement instanceof IASTGotoStatement ){
	        ...
	        if( scope != null && scope instanceof ICFunctionScope ){
	            FortranFunctionScope functionScope = (FortranFunctionScope) scope;
	            ...
	            //label not found
	            return new FortranLabel.CLabelProblem( ((IASTGotoStatement)statement).getName(), IProblemBinding.SEMANTIC_LABEL_STATEMENT_NOT_FOUND, gotoName );
	        }
	    } else if( statement instanceof IASTLabelStatement ){
	        IASTName name = ((IASTLabelStatement)statement).getName();
	        IBinding binding = new FortranLabel( name );
	        try {
	...
	private static IBinding createBinding( ICASTElaboratedTypeSpecifier elabTypeSpec ){
		...
		if( parent instanceof IASTDeclaration ){
			...
			if( binding != null ){
				if( binding instanceof FortranEnumeration ){
			        ((FortranEnumeration)binding).addDeclaration( name );
			    }
			} else {
				if( elabTypeSpec.getKind() == IASTElaboratedTypeSpecifier.k_enum ){
			        binding = new FortranEnumeration( name );
			    } else {
			        binding = new FortranStructure( name );    
			    }
	    ...
	    try{ 
		    if( expression instanceof IASTIdExpression ){
		        ...
		        if( op == IASTUnaryExpression.op_star && (type instanceof IPointerType || type instanceof IArrayType) ){
		            ...
		            return new FortranPointerType( type, 0 );
		        }
		        return type;
		    } else if( expression instanceof IASTLiteralExpression ){
		    	switch( ((IASTLiteralExpression) expression).getKind() ){
		    		case IASTLiteralExpression.lk_char_constant:
		    			return new FortranBasicType( IBasicType.t_char, 0, expression );
		    		case IASTLiteralExpression.lk_float_constant:
		    			return new FortranBasicType( IBasicType.t_float, 0, expression );
		    		case IASTLiteralExpression.lk_integer_constant:
		    			return new FortranBasicType( IBasicType.t_int, 0, expression );
		    		case IASTLiteralExpression.lk_string_literal:
		    			IType type = new FortranBasicType( IBasicType.t_char, 0, expression );
		    			type = new FortranQualifierType( type, true, false, false );
		    			return new FortranPointerType( type, 0 );
		    	}
				...
				switch( op ){
					...
					case IASTBinaryExpression.op_notequals:
						result = new FortranBasicType( IBasicType.t_int, 0 );
						break;
		        ...
		        if( result instanceof FortranBasicType ){
		        	((FortranBasicType)result).setValue( expression );
		        }
		        return result;
		    } else if( expression instanceof IASTUnaryExpression ) {
				int op = ((IASTUnaryExpression)expression).getOperator(); 
				if( op == IASTUnaryExpression.op_sizeof ){
					...
					return new FortranBasicType( IBasicType.t_int, FortranBasicType.IS_LONG | FortranBasicType.IS_UNSIGNED, expression );	
				}
				IType type = getExpressionType(((IASTUnaryExpression)expression).getOperand() );
				...
				if( op == IASTUnaryExpression.op_star && (type instanceof IPointerType || type instanceof IArrayType) ){
				    ...
				    return new FortranPointerType( type, 0 );
				} else if ( type instanceof FortranBasicType ){
					((FortranBasicType)type).setValue( expression );
				}
				return type;
		    }  else if( expression instanceof IASTFieldReference ){
				...
				if( typeidExp.getOperator() == IASTTypeIdExpression.op_sizeof ){
					...
					return new FortranBasicType( IBasicType.t_int, FortranBasicType.IS_LONG | FortranBasicType.IS_UNSIGNED );
				}
	...
	private static IBinding createBinding(IASTDeclarator declarator, IASTName name) {
		...
		if( declarator instanceof ICASTKnRFunctionDeclarator ){
			...
			if ( CharArrayUtils.equals(declarator.getName().toCharArray(), name.toCharArray()) ){
				...
				if( binding != null ) {
				    if( binding instanceof IFortranInternalFunction )
				        ((IFortranInternalFunction)binding).addDeclarator( (ICASTKnRFunctionDeclarator) declarator );
				    else 
			    ...
			    if( f instanceof FortranFunction ){
			        binding = ((FortranFunction) f).resolveParameter( name );
			    }
	...
	private static IBinding createBinding( IASTDeclarator declarator ){
        ...
        if( parent instanceof IASTParameterDeclaration || parent.getPropertyInParent() == ICASTKnRFunctionDeclarator.FUNCTION_PARAMETER ){
		    ...
		    if( temp != null && temp instanceof FortranFunction ){
		        binding = ((FortranFunction) temp).resolveParameter( name );
		    } else if( temp instanceof IFunction ){
		    	 //problems with the function, still create binding for the parameter
			    binding = new FortranParameter( name );
		    }
		    try {
			...
			if( binding != null ) {
			    if( binding instanceof IFunction ){
			        ...
			        if( function instanceof FortranFunction )
			        	((FortranFunction)function).addDeclarator( (IASTFunctionDeclarator) declarator );
			        return function;
			    }
		        binding = new ProblemBinding( name, IProblemBinding.SEMANTIC_INVALID_OVERLOAD, name.toCharArray() );
				...
				binding = null; // TODO - remove new CTypedef( name );
			else
				binding = new FortranFunction( (IASTFunctionDeclarator) declarator );
		} else if( parent instanceof IASTSimpleDeclaration ){
			IASTSimpleDeclaration simpleDecl = (IASTSimpleDeclaration) parent;			
			if( simpleDecl.getDeclSpecifier().getStorageClass() == IASTDeclSpecifier.sc_typedef ){
				binding = null; // TODO - new CTypedef( name );
			} else {
			    ...
			    if( binding != null ) {
			        if( binding instanceof IParameter ){
	                    ...
	                    if( t1 != null && t2 != null && t1.isSameType( t2 ) ){
	    			        if( binding instanceof FortranVariable )
	    			            ((FortranVariable)binding).addDeclaration( name );
	    			    } else {
	    			        return new ProblemBinding( name, IProblemBinding.SEMANTIC_INVALID_REDECLARATION, name.toCharArray() );
	    			    }
			    	}
			    } else if( simpleDecl.getParent() instanceof ICASTCompositeTypeSpecifier ){
					binding = new FortranField( name );
				} else {
					binding = new FortranVariable( name );
				}
			}
		}
	...
	private static IBinding createBinding( ICASTCompositeTypeSpecifier compositeTypeSpec ){
		...
		try {
			...
			binding = scope.getBinding( name, false );
			if( binding != null ){
				if (binding instanceof FortranStructure)
					((FortranStructure)binding).addDefinition( compositeTypeSpec );
				return binding;
			}
		...
		if( node instanceof IASTFunctionDefinition ){
			...
			if ( (blockItem instanceof IASTSimpleDeclaration ||
				...
				if ( simpleDecl != null ) {
					...
					if ( struct instanceof FortranStructure ) {
						try {
                            return ((FortranStructure)struct).findField(((ICASTFieldDesignator)node).getName().toString());
                        } catch ( DOMException e ) {
                            return e.getProblem();
                        }
                        ...
                        try {
                            ...
                            while ( type instanceof ITypeContainer && !(type instanceof FortranStructure) ) {
    							type = ((ITypeContainer)type).getType();
    						}
                        } catch ( DOMException e ) {
                            return e.getProblem();
						...
						if ( type instanceof FortranStructure )
                            try {
                                return ((FortranStructure)type).findField(((ICASTFieldDesignator)node).getName().toString());
                            } catch ( DOMException e1 ) {
	...
	private static IBinding externalBinding( IASTTranslationUnit tu, IASTName name ){
	    ...
	    if( parent instanceof IASTIdExpression ){
	        if( parent.getPropertyInParent() == IASTFunctionCallExpression.FUNCTION_NAME ){
	            //external function
	            external = new FortranExternalFunction( tu, name );
	            ((FortranScope)tu.getScope()).addName( name );
	        } 
	        else {
	            //external variable
	...
	public static IType createType(IASTDeclarator declarator) {
        ...
        if( isParameter ) {
            ...
            if( type instanceof IArrayType ){
	            FortranArrayType at = (FortranArrayType) type;
	            type = new FortranQualifiedPointerType( at.getType(), at.getModifier() );
	        } else if( type instanceof IFunctionType ) {
	            //-8 A declaration of a parameter as "function returning type" shall be adjusted to "pointer to function returning type"
	            type = new FortranPointerType( type, 0 );
	        }
        }
	...
	public static IType createType( IType returnType, IASTFunctionDeclarator declarator){
	    ...
	    IType type = new FortranFunctionType( returnType, pTypes );
	...
	public static IType createBaseType( IASTDeclSpecifier declSpec ) {
		if( declSpec instanceof IGCCASTSimpleDeclSpecifier ){
			...
			return new FortranBasicType( (ICASTSimpleDeclSpecifier) declSpec );
	...
	public static IType createType( ICASTDeclSpecifier declSpec ) {
	    if (declSpec.isConst() || declSpec.isVolatile() || declSpec.isRestrict()) {
			return new FortranQualifierType(declSpec);
		}
	...
	private static IType setupArrayChain(IASTDeclarator decl, IType lastType) {
		if (decl instanceof IASTArrayDeclarator) {
			...
			FortranArrayType arrayType = new FortranArrayType(lastType); 
			...
			for (; i < ((IASTArrayDeclarator)decl).getArrayModifiers().length - 1; i++) {
				arrayType = new FortranArrayType(arrayType);
				if (mods[i] instanceof ICASTArrayModifier) {
					arrayType.setModifiedArrayModifier((ICASTArrayModifier)mods[i]);
				}
			}
	...
	private static IType setupPointerChain(IASTPointerOperator[] ptrs, IType lastType) {
		FortranPointerType pointerType = null;
		
		if ( ptrs != null && ptrs.length > 0 ) {
			pointerType = new FortranPointerType();
											
			if (ptrs.length == 1) {
				pointerType.setType(lastType);
				pointerType.setQualifiers(
						(((ICASTPointer)ptrs[0]).isConst() ? FortranPointerType.IS_CONST : 0) |
						(((ICASTPointer)ptrs[0]).isRestrict() ? FortranPointerType.IS_RESTRICT : 0) |
						(((ICASTPointer)ptrs[0]).isVolatile() ? FortranPointerType.IS_VOLATILE : 0));				
			} else {
				FortranPointerType tempType = new FortranPointerType();
				pointerType.setType(tempType);
				pointerType.setQualifiers(
						(((ICASTPointer)ptrs[ptrs.length - 1]).isConst() ? FortranPointerType.IS_CONST : 0) |
						(((ICASTPointer)ptrs[ptrs.length - 1]).isRestrict() ? FortranPointerType.IS_RESTRICT : 0) |
						(((ICASTPointer)ptrs[ptrs.length - 1]).isVolatile() ? FortranPointerType.IS_VOLATILE : 0));
				int i = ptrs.length - 2;
				for (; i > 0; i--) {
					tempType.setType(new FortranPointerType());
					tempType.setQualifiers(
							(((ICASTPointer)ptrs[i]).isConst() ? FortranPointerType.IS_CONST : 0) |
							(((ICASTPointer)ptrs[i]).isRestrict() ? FortranPointerType.IS_RESTRICT : 0) |
							(((ICASTPointer)ptrs[i]).isVolatile() ? FortranPointerType.IS_VOLATILE : 0));
					tempType = (FortranPointerType)tempType.getType();
				}					
    ...
    public static IBinding[] findBindings( IScope scope, String name, boolean prefixLookup ) throws DOMException{
        ...
        FortranASTName astName = new FortranASTName( name.toCharArray() );
        ...
        do{
            char [] n = name.toCharArray();
            if( scope instanceof ICFunctionScope ){
                ILabel [] labels = ((FortranFunctionScope)scope).getLabels();
                for( int i = 0; i < labels.length; i++ ){
	                ILabel label = labels[i];
	                if (prefixLookup) {

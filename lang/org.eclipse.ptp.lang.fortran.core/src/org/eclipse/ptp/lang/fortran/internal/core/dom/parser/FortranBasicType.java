 ...
 * @author dsteffle
 */
public class FortranBasicType implements ICBasicType {
	 ...
	 */
	public FortranBasicType(ICASTSimpleDeclSpecifier sds) {
		this.type = sds.getType();
		this.qualifiers = ( sds.isLong()    ? FortranBasicType.IS_LONG  : 0 ) |
		   				  ( sds.isShort()   ? FortranBasicType.IS_SHORT : 0 ) |
		   				  ( sds.isSigned()  ? FortranBasicType.IS_SIGNED: 0 ) |
		   				  ( sds.isUnsigned()? FortranBasicType.IS_UNSIGNED : 0 ) |
						  ( sds.isUnsigned()? FortranBasicType.IS_LONGLONG : 0 ) |
						  ( sds.isComplex() ? FortranBasicType.IS_COMPLEX : 0 ) |
						  ( sds.isImaginary()?FortranBasicType.IS_IMAGINARY : 0 );
	...
	public FortranBasicType( int type, int qualifiers ){
		this.type = type;
	...
	public FortranBasicType( int type, int qualifiers, IASTExpression value ){
		this.type = type;
		this.qualifiers = qualifiers;
		this.value = value;
	}
	...
	public boolean isSameType(IType obj) {
		...
		if (!(obj instanceof FortranBasicType)) return false;
		
		FortranBasicType cObj = (FortranBasicType)obj;

 ...
 **********************************************************************/
package org.eclipse.cldt.utils.xcoff;
...
import org.eclipse.cldt.utils.coff.ReadMemoryAccess;
...
//		public final static int F_AR32W = 0x0200;  // file is 32-bit big-endian
		...
		public void commonSetup(byte[] hdr, boolean little) throws IOException {
			if (hdr == null || hdr.length < FILHSZ) {
				throw new EOFException(CommonLanguageCore.getResourceString("Util.exception.arrayToSmall")); //$NON-NLS-1$
			}
			if (!isXCOFF32Header(hdr)) {
				throw new IOException(CommonLanguageCore.getResourceString("Util.exception.notXCOFF32")); //$NON-NLS-1$
			}

...
package org.eclipse.cldt.utils.coff;
...
import org.eclipse.cldt.utils.Addr32Factory;
import org.eclipse.cldt.utils.coff.Coff.FileHeader;
import org.eclipse.cldt.utils.coff.Coff.OptionalHeader;
import org.eclipse.cldt.utils.coff.Coff.SectionHeader;
import org.eclipse.cldt.utils.coff.Coff.Symbol;
import org.eclipse.cldt.utils.coff.Exe.ExeHeader;
	...
	public PE (String filename, long pos, boolean filter) throws IOException {
		try {
			...
			try {
				...
				if (!((sig[0] == 'P') && (sig[1] == 'E')
				   && (sig[2] == '\0') && (sig[3] == '\0'))) {
					throw new IOException(CommonLanguageCore.getResourceString("Util.exception.notPE")); //$NON-NLS-1$
				}
			} catch (IOException e) {
				rfile.seek(pos);
			}
			...
			if (!isValidMachine(fileHeader.f_magic)) {
				throw new IOException(CommonLanguageCore.getResourceString("Util.exception.unknownFormat")); //$NON-NLS-1$
			}

...
package org.eclipse.cldt.utils.debug.dwarf;
...
public class Dwarf {
	...
	int read_4_bytes(InputStream in) throws IOException {
		try {
			...
			if (n != 4) {
				throw new IOException(CommonLanguageCore.getResourceString("Util.exception.missingBytes")); //$NON-NLS-1$
			}
	...
	long read_signed_leb128(InputStream in) throws IOException {
		...
		while (true) {
			b = (short) in.read();
			if (b == -1)
				throw new IOException(CommonLanguageCore.getResourceString("Util.exception.noData")); //$NON-NLS-1$
			num_leb128_read++;
			result |= ((long) (b & 0x7f) << shift);
			shift += 7;
			if ((b & 0x80) == 0) {

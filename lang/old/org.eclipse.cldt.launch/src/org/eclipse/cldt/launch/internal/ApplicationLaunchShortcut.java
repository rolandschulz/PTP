...
***********************************************************************/
package org.eclipse.cldt.launch.internal;
...
import org.eclipse.cldt.launch.AbstractLaunchDelegate;
...
public class ApplicationLaunchShortcut implements ILaunchShortcut {
	...
	protected ILaunchConfiguration findLaunchConfiguration(IBinary bin, String mode) {
		...
		if (candidateCount < 1) {
			...
			for (int i = 0; i < debugConfigs.length; i++) {
				...
				if (debugConfigs[i].supportsMode(IFDTLaunchConfigurationConstants.DEBUGGER_MODE_RUN)) {
	...
	private ILaunchConfiguration createConfiguration(IBinary bin, ICDebugConfiguration debugConfig) {
		...
		try {
			...
			wc.setAttribute(IFDTLaunchConfigurationConstants.ATTR_PROGRAM_NAME, projectName);
			wc.setAttribute(IFDTLaunchConfigurationConstants.ATTR_PROJECT_NAME, bin.getCProject().getElementName());
			wc.setAttribute(IFDTLaunchConfigurationConstants.ATTR_WORKING_DIRECTORY, (String) null);
			wc.setAttribute(IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_STOP_AT_MAIN, true);
			wc.setAttribute(
				IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_START_MODE,
				IFDTLaunchConfigurationConstants.DEBUGGER_MODE_RUN);
			wc.setAttribute(IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ID, debugConfig.getID());
			config = wc.doSave();

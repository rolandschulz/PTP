...
public class DebuggerTab extends AbstractDebuggerTab {
	...
	public class AdvancedDebuggerOptionsDialog extends Dialog {
		...
		private void initialize() {
			...
			Object varBookkeeping = attr.get( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING );
			fVarBookKeeping.setSelection( ( varBookkeeping instanceof Boolean ) ? !((Boolean)varBookkeeping).booleanValue() : true );
			Object regBookkeeping = attr.get( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING );
		...
		private void saveValues() {
			...
			attr.put( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING, varBookkeeping );
			Boolean regBookkeeping = ( fRegBookKeeping.getSelection() ) ? Boolean.FALSE : Boolean.TRUE;
			attr.put( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, regBookkeeping );
	...
	protected void loadDebuggerComboBox( ILaunchConfiguration config, String selection ) {
		...
		String mode;
		if (fAttachMode) {
			mode = IFDTLaunchConfigurationConstants.DEBUGGER_MODE_ATTACH;
		} else {
			mode = IFDTLaunchConfigurationConstants.DEBUGGER_MODE_RUN;  
		}
	...
	public void setDefaults( ILaunchConfigurationWorkingCopy config ) {
		...
		if (fAttachMode) {
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_START_MODE, IFDTLaunchConfigurationConstants.DEBUGGER_MODE_ATTACH );
		} else {
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_START_MODE, IFDTLaunchConfigurationConstants.DEBUGGER_MODE_RUN );
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_STOP_AT_MAIN, IFDTLaunchConfigurationConstants.DEBUGGER_STOP_AT_MAIN_DEFAULT );
		}
		config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING, false );
		config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, false );
	}
	...
	public void performApply( ILaunchConfigurationWorkingCopy config ) {
		...
		if ( fAttachMode ) {
			...
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_STOP_AT_MAIN, fStopInMain.getSelection() );
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_START_MODE, IFDTLaunchConfigurationConstants.DEBUGGER_MODE_RUN );
		}
	...
	private void initializeAdvancedAttributes( ILaunchConfiguration config ) {
		Map attr = getAdvancedAttributes();
		try {
			Boolean varBookkeeping = ( config.getAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING, false ) ) ? Boolean.TRUE : Boolean.FALSE;
			attr.put( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING, varBookkeeping );
		}
		catch( CoreException e ) {
		}
		try {
			Boolean regBookkeeping = ( config.getAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, false ) ) ? Boolean.TRUE : Boolean.FALSE;
			attr.put( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, regBookkeeping );
		}
		catch( CoreException e ) {
		}
	...
	private void applyAdvancedAttributes( ILaunchConfigurationWorkingCopy config ) {
		...
		if ( varBookkeeping instanceof Boolean )
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_VARIABLE_BOOKKEEPING, ((Boolean)varBookkeeping).booleanValue() );
		Object regBookkeeping = attr.get( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING );
		if ( regBookkeeping instanceof Boolean )
			config.setAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_ENABLE_REGISTER_BOOKKEEPING, ((Boolean)regBookkeeping).booleanValue() );
	}
	...
	protected void initializeCommonControls( ILaunchConfiguration config ) {
		try {
			if (!fAttachMode) {
				fStopInMain.setSelection( config.getAttribute( IFDTLaunchConfigurationConstants.ATTR_DEBUGGER_STOP_AT_MAIN, IFDTLaunchConfigurationConstants.DEBUGGER_STOP_AT_MAIN_DEFAULT ) );
			}
	...
	/* (non-Javadoc)
	 * @see org.eclipse.cdt.launch.internal.ui.AbstractDebuggerTab#setInitializeDefault(boolean)
	 */

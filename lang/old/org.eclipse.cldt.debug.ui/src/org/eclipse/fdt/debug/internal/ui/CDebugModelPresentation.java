...
import org.eclipse.fdt.debug.core.cdi.ICDIExitInfo;
...
import org.eclipse.fdt.debug.core.model.ICFunctionBreakpoint;
...
import org.eclipse.fdt.debug.core.model.ICThread;
import org.eclipse.fdt.debug.core.model.ICType;
import org.eclipse.fdt.debug.core.model.ICValue;
import org.eclipse.fdt.debug.core.model.ICVariable;
import org.eclipse.fdt.debug.core.model.ICWatchpoint;
import org.eclipse.fdt.debug.core.model.IDummyStackFrame;
...
import org.eclipse.fdt.debug.internal.ui.editors.CDebugEditor;
import org.eclipse.fdt.debug.internal.ui.editors.EditorInputDelegate;
...
public class CDebugModelPresentation extends LabelProvider implements IDebugModelPresentation {
	...
	public String getText( Object element ) {
		...
		if ( element instanceof IAdaptable ) {
			...
			if ( target != null ) {
				if ( !target.isEnabled() ) {
					baseText.append( ' ' );
					baseText.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.25" ) ); //$NON-NLS-1$
				}
			}
		}
		...
		try {
			...
			if ( element instanceof ITerminate ) {
				if ( ((ITerminate)element).isTerminated() ) {
					label.insert( 0, CDebugUIMessages.getString( "FDTDebugModelPresentation.0" ) ); //$NON-NLS-1$
					return label.toString();
				}
			}
			if ( element instanceof IDisconnect ) {
				if ( ((IDisconnect)element).isDisconnected() ) {
					label.insert( 0, CDebugUIMessages.getString( "FDTDebugModelPresentation.1" ) ); //$NON-NLS-1$
					return label.toString();
				}
			}
			if ( label.length() > 0 ) {
			...
			return MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.2" ), new String[] { e.getMessage() } ); //$NON-NLS-1$
		}
		catch( CoreException e ) {
	...
	protected StringBuffer appendLineNumber( ICLineBreakpoint breakpoint, StringBuffer label ) throws CoreException {
		...
		if ( lineNumber > 0 ) {
			label.append( ' ' );
			label.append( MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.26" ), new String[]{ Integer.toString( lineNumber ) } ) ); //$NON-NLS-1$
		}
		return label;
	}
	...
	protected StringBuffer appendAddress( ICAddressBreakpoint breakpoint, StringBuffer label ) throws CoreException {
		try {
			label.append( ' ' );
			label.append( MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.27" ), new String[]{ breakpoint.getAddress() } ) ); //$NON-NLS-1$
		}
	...
	protected StringBuffer appendFunction( ICFunctionBreakpoint breakpoint, StringBuffer label ) throws CoreException {
		...
		if ( function != null && function.trim().length() > 0 ) {
			label.append( ' ' );
			label.append( MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.28" ), new String[]{ function.trim() } ) ); //$NON-NLS-1$
		}
		return label;
	}
	...
	protected StringBuffer appendIgnoreCount( ICBreakpoint breakpoint, StringBuffer label ) throws CoreException {
		...
		if ( ignoreCount > 0 ) {
			label.append( ' ' );
			label.append( MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.29" ), new String[]{ Integer.toString( ignoreCount ) } ) ); //$NON-NLS-1$
		}
		return label;
	}
	...
	protected void appendCondition( ICBreakpoint breakpoint, StringBuffer buffer ) throws CoreException {
		...
		if ( condition != null && condition.length() > 0 ) {
			buffer.append( ' ' );
			buffer.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.30" ) ); //$NON-NLS-1$
			buffer.append( ' ' );
	...
	private void appendWatchExpression( ICWatchpoint watchpoint, StringBuffer label ) throws CoreException {
		...
		if ( expression != null && expression.length() > 0 ) {
			label.append( ' ' );
			label.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.31" ) ); //$NON-NLS-1$
	...
	protected String getValueText( IValue value )/* throws DebugException*/ {
		...
		if ( value instanceof ICDebugElementStatus && !((ICDebugElementStatus)value).isOK() ) {
			label.append(  getFormattedString( CDebugUIMessages.getString( "FDTDebugModelPresentation.4" ), ((ICDebugElementStatus)value).getMessage() ) ); //$NON-NLS-1$
		}
				...
				if ( valueString != null ) {
					...
					if ( type != null && type.isCharacter() ) {
						...
						if ( CDebugUtils.isPositiveInfinity( floatingPointValue ) )
							valueString = CDebugUIMessages.getString( "FDTDebugModelPresentation.23" ); //$NON-NLS-1$
						if ( CDebugUtils.isNegativeInfinity( floatingPointValue ) )
							valueString = CDebugUIMessages.getString( "FDTDebugModelPresentation.24" ); //$NON-NLS-1$
						label.append( valueString );
					}
					else if ( type == null || (!type.isArray() && !type.isStructure()) ) {
						if ( valueString.length() > 0 ) {
	...
	protected String getSignalText( ICSignal signal ) {
		StringBuffer sb = new StringBuffer( CDebugUIMessages.getString( "FDTDebugModelPresentation.12" ) ); //$NON-NLS-1$
		try {
	...
	protected String getWatchExpressionText( IWatchExpression expression ) {
		...
		if ( !expression.isEnabled() ) {
			result.append( ' ' );
			result.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.22" ) ); //$NON-NLS-1$
		}
		return result.toString();
	}
		...
		if ( t != null ) {
			if ( !t.isPostMortem() ) {
				...
				if ( state.equals( CDebugElementState.EXITED ) ) {
					...
					String label = CDebugUIMessages.getString( "FDTDebugModelPresentation.3" ); //$NON-NLS-1$
					String reason = ""; //$NON-NLS-1$
					if ( info != null && info instanceof ICDISignalExitInfo ) {
						ICDISignalExitInfo sigInfo = (ICDISignalExitInfo)info;
						reason = ' ' + MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.5" ), new String[]{ sigInfo.getName(), sigInfo.getDescription() } ); //$NON-NLS-1$
					}
					else if ( info != null && info instanceof ICDIExitInfo ) {
						reason = ' ' + MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.6" ), new Integer[] { new Integer( ((ICDIExitInfo)info).getCode() ) } ); //$NON-NLS-1$
					}
					return MessageFormat.format( label, new String[] { target.getName(), reason } );
				}
				else if ( state.equals( CDebugElementState.SUSPENDED ) ) {
						return MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.7" ), new String[] { target.getName() } ); //$NON-NLS-1$
				}
			}
		}
	...
	protected String getThreadText( IThread thread, boolean qualified ) throws DebugException {
		...
		if ( target.isPostMortem() ) {
			return getFormattedString( CDebugUIMessages.getString( "FDTDebugModelPresentation.8" ), thread.getName() ); //$NON-NLS-1$
		}
		if ( thread.isTerminated() ) {
			return getFormattedString( CDebugUIMessages.getString( "FDTDebugModelPresentation.9" ), thread.getName() ); //$NON-NLS-1$
		}
		if ( thread.isStepping() ) {
			return getFormattedString( CDebugUIMessages.getString( "FDTDebugModelPresentation.10" ), thread.getName() ); //$NON-NLS-1$
		}
		if ( !thread.isSuspended() ) {
			return getFormattedString( CDebugUIMessages.getString( "FDTDebugModelPresentation.11" ), thread.getName() ); //$NON-NLS-1$
		}
		if ( thread.isSuspended() ) {
			String reason = ""; //$NON-NLS-1$
			...
			if ( element != null ) {
				...
				if ( info != null && info instanceof ICDISignalReceived ) {
					...
					reason = MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.13" ), new String[]{ signal.getName(), signal.getDescription() } ); //$NON-NLS-1$
				}
				else if ( info != null && info instanceof ICDIWatchpointTrigger ) {
					reason = MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.14" ), new String[]{ ((ICDIWatchpointTrigger)info).getOldValue(), ((ICDIWatchpointTrigger)info).getNewValue() } ); //$NON-NLS-1$
				}
				else if ( info != null && info instanceof ICDIWatchpointScope ) {
					reason = CDebugUIMessages.getString( "FDTDebugModelPresentation.15" ); //$NON-NLS-1$
				}
				else if ( info != null && info instanceof ICDIBreakpointHit ) {
					reason = CDebugUIMessages.getString( "FDTDebugModelPresentation.16" ); //$NON-NLS-1$
				}
				else if ( info != null && info instanceof ICDISharedLibraryEvent ) {
					reason = CDebugUIMessages.getString( "FDTDebugModelPresentation.17" ); //$NON-NLS-1$
				}
			}
			return MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.18" ), new String[] { thread.getName(), reason } ); //$NON-NLS-1$
		}
		return MessageFormat.format( CDebugUIMessages.getString( "FDTDebugModelPresentation.19" ), new String[] { thread.getName() } ); //$NON-NLS-1$
	}
	...
	protected String getStackFrameText( IStackFrame f, boolean qualified ) throws DebugException {
		if ( f instanceof ICStackFrame ) {
			...
			if ( function != null ) {
				...
				if ( function.length() > 0 ) {
					...
					if ( frame.getFile() != null ) {
						...
						if ( !path.isEmpty() ) {
							label.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.20" ) ); //$NON-NLS-1$
							label.append( ' ' );
							label.append( (qualified ? path.toOSString() : path.lastSegment()) );
							label.append( ':' );
			...
			if ( isEmpty( function ) )
				label.append( CDebugUIMessages.getString( "FDTDebugModelPresentation.21" ) ); //$NON-NLS-1$
			return label.toString();
		}

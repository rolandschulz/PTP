...
public class FilterDescriptor implements Comparable {
	...
	FilterDescriptor(IConfigurationElement element) {
		...
		Assert.isTrue(isPatternFilter() ^ isCustomFilter(), "An extension for extension-point org.eclipse.cldt.ui.CElementFilters does not specify a correct filter"); //$NON-NLS-1$
		Assert.isNotNull(getId(), "An extension for extension-point org.eclipse.cldt.ui.CElementFilters does not provide a valid ID"); //$NON-NLS-1$
		Assert.isNotNull(getName(), "An extension for extension-point org.eclipse.cldt.ui.CElementFilters does not provide a valid name"); //$NON-NLS-1$
	}

...
public class WildcardResourceMatcher extends ResourceMatcher {
	...
	public boolean match(IResource candidate) {
		...
		if(DEBUG)System.out.println("Wildcard match: candidate: "+candidate+" - "+candidate.getProjectRelativePath().toOSString());  //$NON-NLS-1$//$NON-NLS-2$
		boolean result= matches(candidate.getProjectRelativePath().toOSString(),wildcard);
		
		if(DEBUG)System.out.println("       to pattern: "+wildcard+" is: "+result);  //$NON-NLS-1$//$NON-NLS-2$
		return result;
	}
	/**
	...
	private boolean matches(String text, String wildcardPattern) {	
		...
		for (int i = 0; i < wildcardPattern.length(); i++)
			if (wildcardPattern.charAt(i) != '?'
					&& !wildcardPattern.substring(i, i + 1).equalsIgnoreCase(text.substring(i, i + 1)))
				return false;

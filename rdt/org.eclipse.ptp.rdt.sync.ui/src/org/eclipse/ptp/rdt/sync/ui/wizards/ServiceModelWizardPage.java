...
public class ServiceModelWizardPage extends MBSCustomPage {
	public static final String SERVICE_MODEL_WIZARD_PAGE_ID = "org.eclipse.ptp.rdt.sync.ui.serviceModelWizardPage"; //$NON-NLS-1$
	public static final String DEFAULT_CONFIG = Messages.ConfigureRemoteServices_0;
	/**
	 * @since 2.0
	 */
	public static final String CONFIG_PROPERTY = "org.eclipse.ptp.rdt.sync.ui.ServiceModelWizardPage.serviceConfig"; //$NON-NLS-1$
	...
	public String getName() {
		return Messages.ServiceModelWizardPage_0;
	}
	...
	private IServiceConfiguration getNewConfiguration() {
		if (fNewConfig == null) {
			...
			if (page instanceof NewRemoteProjectCreationPage) {
				...
				// smm.getService(IRDTServiceConstants.SERVICE_C_INDEX);
				//				if (remoteServices.getId().equals("org.eclipse.ptp.remote.RSERemoteServices")) { //$NON-NLS-1$
				// descriptor =
				// indexingService.getProviderDescriptor(RSECIndexServiceProvider.ID);
				// RSECIndexServiceProvider provider =
				// (RSECIndexServiceProvider)
				// smm.getServiceProvider(descriptor);
				// if (provider != null) {
				...
				//
				// provider.setConnection(host,
				// getDStoreConnectorService(host));
				...
				// (IRemoteToolsIndexServiceProvider) smm
	...
	public String getDescription() {
		if (fDescription == null)
			fDescription = Messages.ServiceModelWizardPage_description;
		return fDescription;
	}
	...
	public String getTitle() {
		if (fTitle == null)
			fTitle = Messages.ServiceModelWizardPage_0;
		return fTitle;
	}

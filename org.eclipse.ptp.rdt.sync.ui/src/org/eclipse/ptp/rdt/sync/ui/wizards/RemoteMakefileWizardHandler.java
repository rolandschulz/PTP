...
/**
 * This class handles what happens during project creation when the user has
 * selected "Remote Makefile Project".
 * 
...
public class RemoteMakefileWizardHandler extends STDWizardHandler {
	 ...
	 * 
	 * @see
	 * org.eclipse.cdt.managedbuilder.ui.wizards.STDWizardHandler#createProject
	 * (org.eclipse.core.resources.IProject, boolean, boolean,
	...
	public void createProject(IProject project, boolean defaults, boolean onFinish, IProgressMonitor monitor) throws CoreException {
		try {
			...
			// set the build directory by default to be that of the project...
			// the usual workspace macro doesn't work as the workspace resides
			// locally
			// and the project resides remotely

			URI projectLocation = project.getLocationURI();
			// assume that the path portion of the URI corresponds to the path
			// on the remote machine
			// this may not work if the remote machine does not use UNIX paths
			// but we have no real way of knowing the path

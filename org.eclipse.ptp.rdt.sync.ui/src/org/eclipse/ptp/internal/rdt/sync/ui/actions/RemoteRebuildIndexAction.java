...
package org.eclipse.ptp.internal.rdt.sync.ui.actions;
	...
	public void run(IAction action) {
		...
		for (Iterator<?> i = cElements.iterator(); i.hasNext();) {
			...
			if (elem instanceof ICProject) {
				if (RemoteSyncNature.hasRemoteNature(((ICProject) elem).getProject())) {
					// IndexBuildSequenceController indexBuildSequenceController
					// =
					// IndexBuildSequenceController.getIndexBuildSequenceController(((ICProject)elem).getProject());

					Shell shell = new Shell();

					// if (indexBuildSequenceController.isOptionalIndex()) {
					// boolean continueIndex = MessageDialog
					// .openQuestion(
					// shell,
					//										Messages.getString("RemoteRebuildIndexOption.question"), Messages.getString("RemoteRebuildIndexOption.message")); //$NON-NLS-1$ //$NON-NLS-2$
					// if (continueIndex) {

...
public class MPIMVAnalysis {
		...
		for (ICallGraphNode n = cg_.topEntry(); n != null; n = n.topNext()) {
			...
			for (IBlock b = cfg.getEntry(); b != null; b = b.topNext()) {
				...
				if (block.getMV() && (block.withBreak || block.withContinue))
					System.out.println("************  Multi-valued branch with Break/Continue in " + node.getFuncName() + "("
							+ node.getFileName() + ")");
			}
		}
	}
	...
	class ExprMVAnalyzer {
		...
		/**
		 * Return true if "expr" is multi_valued. "set" contains all defined
		 * variables in "expr" through assignment
		 */
			...
			if (expr instanceof IASTAmbiguousExpression) {
				...
				if (n != null) {
					...
					if (parameter != null) {
						...
						// this probably still
						// works ok here.
						if (parameter instanceof IASTExpressionList) { // >1
																		// parameter
					...
					for (Iterator<String> i = n.getGlobalUse().iterator(); i.hasNext();) {
						...
						if (context_.contains(guse)) {
							...
							Util.addAll(MVvar_, mvlist);
							if (mvlist.contains(n.getFuncName())) { // return MV
																	// value
								MVvar_.remove(n.getFuncName());
								returnval = true;
							}
					...
					if (n.getMVSummary().size() == 1) {// no parameter, no
														// global use
	...
	/**
	 * Work list contains two kinds of blocks: (1) where MPI_Comm_rank() is
	 * directly or indirectly called; (2) the entry block of a function that has
	 * some MV real parameters;
	 * 
	 * (1) is marked in SeedsCollector and collected in WorkListCollector (2) is
	 * collected in the slicing function.
	 */
	class WorkListCollector extends ASTVisitor {
			...
			if (expr instanceof IASTFunctionCallExpression) {
				...
				if (inRankFunc) {
					...
					params_ = initClause;
															...
															// 7.0 change) are
															// here
				// BRT re: CDT 7.0 changes for barrier matching bug: still not
				...
				if (inRankFunc) {
					...
					while (true) {
						...
						if (parent instanceof IASTFunctionCallExpression)
							...
							if (traceOn)
								System.out.println("MMVA: me=parent, parent=parent.getParent()");
							me = parent;
		...
		public int leave(IASTExpression expr) {
			if (expr instanceof IASTFunctionCallExpression) {
				...
				// BRT IASTFunctionCallExpr is one level higher in the AST than
				// the old IASTExpressionList was
			} else if (expr instanceof IASTExpressionList) {
				paramsOLD = null;
			}
			return PROCESS_CONTINUE;
	...
	private void exprMVAnalysis() {
		for (ICallGraphNode n = cg_.botEntry(); n != null; n = n.botNext()) {
			...
			for (IBlock b = cfg.getEntry(); b != null; b = b.topNext()) {
				MPIBlock block = (MPIBlock) b;
				// BRT System.out.println("expMVAnalysis(): Block " +
				// block.getID());

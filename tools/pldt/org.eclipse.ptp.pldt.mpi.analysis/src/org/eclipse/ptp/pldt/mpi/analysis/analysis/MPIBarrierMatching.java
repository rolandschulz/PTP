...
public class MPIBarrierMatching {
	 ...
	 * 
	 * @param reportErrors
	 *            if true, produces a popup warning of barrier errors when
	 *            found. <br>
	 *            if false, no popup occurs. Useful for running under JUnit
	 *            test, with no user interaction. Note that reportErrors is also
	 *            used to make sure only a single error is reported - the popup
	 *            warning, if shown, is only shown once per analysis.
	 * @since 4.0
	 */
	public void run(boolean reportErrors) {
		for (ICallGraphNode n = cg_.botEntry(); n != null; n = n.botNext()) {
			...
			for (Enumeration<String> e = barrierExpr.keys(); e.hasMoreElements();) {
				currentComm_ = e.nextElement();
				BarrierExpression be = barrierExpr.get(currentComm_);
				error = false;
				fixedLength(be);
				if (error) {
					// Don't report an error dialog for *each* barrier matching
					// error found
					...
					if (reportErrors) {
						...
						MessageDialog.openInformation(ShowMatchSet.getStandardDisplay().getActiveShell(), "MPI Barrier Analysis",
								errorMsg);
					}
		...
		if (traceOn)
			printMatches();
		if (traceOn)
			System.out.println("mv = " + mv + ", sv = " + sv);
	}
	...
	protected void reportWarning(BarrierExpression BE) {
		...
		if (traceOn)
			System.out.println("Barrier mismatching found in " + stmt);
		...
		ErrorMessage err = new ErrorMessage(cond, stmt, currentFunc_.getFuncName(), currentFunc_.getFileName(),
				currentFunc_.getResource());
	...
	protected void traverseBarrierExpr(List<PathNode> path, BarrierExpression BE, IControlFlowGraph cfg) {
		...
		if (op == null) {
			...
			if (cond.getMV()) {
				 ...
				 * thus it doesn't matter which branch to choose. By default we
	...
	public class PathNode {
		private final BarrierInfo barrier_;
	...
	public class ErrorMessage {
		private final IASTExpression position_;
		private final IASTStatement errStmt_;
		private final String funcName_;
		private final String fileName_;
		private final SourceInfo sourceInfo_;
		private SourceInfo path1SourceInfo_;
		private SourceInfo path2SourceInfo_;
		private final IResource resource_;
		...
		SourceInfo getSourceInfo(IASTNode node) {
			...
			if (locations.length == 1) {
				...
				// CASTCompoundStatement causes this. Probably ok? assuming it
				// descends into the child nodes
				if (traceOn) {
	...
	protected void match(BarrierExpression BE1, BarrierExpression BE2, int dir) {
		...
		while (true) {
			if (traceOn)
				System.out.println("match: " + T1.prettyPrinter());
			if (traceOn)
				System.out.println("       " + T2.prettyPrinter());
			if (direction == down) {
				if (traceOn)
			...
			if (direction == up && T1 == BE1 && T2 == BE2)
				break;
			else if (direction == down && OP1 == null && OP2 == null && T1.isBot() && T2.isBot())
				direction = up;
			else if (direction == down && OP1 == null && OP2 == null && T1.isBarrier() && T2.isBarrier()) {
				...
				sstack_.push(T2);
				T2 = fBE;
			} else if (direction == down && OP1 != null && OP1.getOperator() == BarrierExpressionOP.op_concat) { // rule
																													// 2
				T1 = T1.getOP1();
			} else if (direction == down && OP2 != null && OP2.getOperator() == BarrierExpressionOP.op_concat) { // rule
																													// 2
				T2 = T2.getOP1();
				...
				T1 = T1.getParent();
				direction = up;
			} else if (direction == up && T2.getParent().getOP().getOperator() == BarrierExpressionOP.op_concat
					&& T2 == T2.getParent().getOP2()) { // rule 3
				T2 = T2.getParent();
				direction = up;
			} else if (direction == up && T1.getParent().getOP().getOperator() == BarrierExpressionOP.op_concat
					&& T1 == T1.getParent().getOP1() && T2.getParent().getOP().getOperator() == BarrierExpressionOP.op_concat
					&& T2 == T2.getParent().getOP1()) { // rule 4
				T1 = T1.getParent().getOP2();
				T2 = T2.getParent().getOP2();
				direction = down;
			} else if (direction == down && OP1 != null && OP1.getOperator() == BarrierExpressionOP.op_branch) { // rule
																													// 5
				workList_.add(new Work(T1.getOP2(), T2, down));
				T1 = T1.getOP1();
			} else if (direction == down && OP2 != null && OP2.getOperator() == BarrierExpressionOP.op_branch) { // rule
																													// 5
				workList_.add(new Work(T1, T2.getOP2(), down));
				T2 = T2.getOP1();
			} else if (direction == up && T1.getParent().getOP().getOperator() == BarrierExpressionOP.op_branch) {
				// rule 6
				T1 = T1.getParent();
			} else if (direction == up && T2.getParent().getOP().getOperator() == BarrierExpressionOP.op_branch) {
				// rule 6
				T2 = T2.getParent();
			} else {
				System.out.println("cannot find rules for " + T1.prettyPrinter() + " and " + T2.prettyPrinter()
		...
		for (Enumeration<List<BarrierInfo>> e = barrierTable_.getTable().elements(); e.hasMoreElements();) {
			...
			for (Iterator<BarrierInfo> i = list.iterator(); i.hasNext();) {
				...
				for (Iterator<BarrierInfo> ii = bar.getMatchingSet().iterator(); ii.hasNext();) {
					...
					if (traceOn)
						System.out.println("matchedBar = " + matchedBar.getID());
					if (!matchedBar.getMatchingSet().contains(bar)) {
						if (traceOn)
							System.out.println("bar = " + bar.getID() + ", matchedBar = " + matchedBar.getID());
	...
	protected void printMatches() {
		for (Enumeration<List<BarrierInfo>> e = barrierTable_.getTable().elements(); e.hasMoreElements();) {
			...
			for (Iterator<BarrierInfo> i = list.iterator(); i.hasNext();) {
				BarrierInfo bar = i.next();
				if (traceOn)
					System.out.print("Barrier " + bar.getID() + " matches to : ");
				for (Iterator<BarrierInfo> ii = bar.getMatchingSet().iterator(); ii.hasNext();) {
					BarrierInfo matchedBar = ii.next();
					if (traceOn)
						System.out.print(matchedBar.getID() + ", ");
				}
			}
			if (traceOn)
				System.out.println(" ");

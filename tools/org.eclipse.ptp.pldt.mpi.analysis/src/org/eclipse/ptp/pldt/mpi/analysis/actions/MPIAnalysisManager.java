...
public class MPIAnalysisManager {
	...
	public boolean run(boolean reportErrors) {
		if (traceOn)
			System.out.println("MPI AM: 1. build CG");
		cg_.buildCG();
		// cg_.print();
		/*
		...
		if (traceOn)
			System.out.println("2. Call Graph finished !");

		MPIBarrierExpr BE = new MPIBarrierExpr(btable_, cg_);
		BE.run();
		if (traceOn)
			System.out.println("3. Barrier Expression Construction finished !");

		MPICFGBuilder cfg = new MPICFGBuilder(cg_);
		cfg.run();
		if (traceOn)
			System.out.println("4. Control Flow Graph Construction finished !");

		MPIDUChain rd = new MPIDUChain(cg_);
		rd.run();
		if (traceOn)
			System.out.println("5. Reaching Definition and Phi Placement finished !");

		MPIMVAnalysis mva = new MPIMVAnalysis(cg_);
		mva.run();
		if (traceOn)
			System.out.println("6. Multi-valued Analysis finished !");

		MPIBarrierMatching bm = new MPIBarrierMatching(cg_, btable_);
		bm.run(reportErrors);
		if (traceOn)
			System.out.println("7. Barrier Matching finished! ");

		MPIBarrierAnalysisResults results = new MPIBarrierAnalysisResults();
		results.setBarrierTable(btable_);
		if (traceOn)
			System.out.println("8. ...got barrier table ");
		...
		ba.run();
		if (traceOn)
			System.out.println("9. ... got barrier artifacts ");

		MatchingSet msv = new MatchingSet(btable_);
		msv.run();
		if (traceOn)
			System.out.println("10. ...got matching set ");

		ShowErrors se = new ShowErrors(bm.getErrors());
		boolean foundError = se.run();
		if (traceOn)
			System.out.println("11.  ... got errors (if any) ");

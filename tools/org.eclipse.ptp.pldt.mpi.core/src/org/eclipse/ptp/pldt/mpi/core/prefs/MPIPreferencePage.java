...
/**
 * Preference page for MPI settings <br>
 ...
 * <p>
 * This should probably be re-evaluated in the future. An alternative might be a "don't ask me again during this run" checkbox on
 * the popup asking to add the API, that appears during execution. That would remove the annoying part; but performance may still
 * suffer from this extra test?
 * <p>
 * Crude workaround: Run MPI analysis at least once on plain C code, to assure it's finding the header files in the same place that
 * are specified here in the preferences.
 * 
 */
...
public class MPIPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage
{
	...
	protected void createFieldEditors()
	{
		...
		BooleanFieldEditor bed = new BooleanFieldEditor(MpiIDs.MPI_PROMPT_FOR_OTHER_INCLUDES, PROMPT_FOR_OTHERS_LABEL,
				getFieldEditorParent());
		addField(bed);

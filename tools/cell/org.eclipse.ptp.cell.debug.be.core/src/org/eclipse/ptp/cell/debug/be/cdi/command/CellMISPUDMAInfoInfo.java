 ...
 *
 */
public class CellMISPUDMAInfoInfo extends MIInfo {
		...
		if (isDone()) {
			...
			if (rr != null) {
				...
				for (int i = 0; i < results.length; i++) {
					...
					if (var.equals("SPUInfoDMA")) { //$NON-NLS-1$
	...
	protected void parseOutterTuple(MITuple tuples) {
		...
		for (int j = 0; j < tables.length; j++) {
			...
			} else if (tableName.equals("dma_info_type")) { //$NON-NLS-1$
				bList.add(new MISPUDMAElement(tables[j]));
			} else if (tableName.equals("dma_info_mask")) { //$NON-NLS-1$
				bList.add(new MISPUDMAElement(tables[j]));
			} else if (tableName.equals("dma_info_status")) { //$NON-NLS-1$
				bList.add(new MISPUDMAElement(tables[j]));
			} else if (tableName.equals("dma_info_stall_and_notify")) { //$NON-NLS-1$
				bList.add(new MISPUDMAElement(tables[j]));
			} else if (tableName.equals("dma_info_atomic_command_status")) { //$NON-NLS-1$
				bList.add(new MISPUDMAElement(tables[j]));
			}
		...
		dmacmds = (MISPUDMAElement[]) bList.toArray(new MISPUDMAElement[] {});
	}		

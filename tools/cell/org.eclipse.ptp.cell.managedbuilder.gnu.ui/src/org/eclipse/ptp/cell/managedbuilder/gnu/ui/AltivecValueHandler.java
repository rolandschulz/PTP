...
import org.eclipse.ptp.cell.managedbuilder.gnu.ui.internal.GnuManagedMakeMessages;
 ...
 * 
 */
public class AltivecValueHandler extends CellDefaultOptionValueHandler {

	public AltivecValueHandler() {
		...
		if (event == EVENT_APPLY) {
			try {
				...
				if (option.getBooleanValue() == true) {
					...
					if (otherOption.getBooleanValue() == false) {
						MessageDialog
								.openWarning(
										PlatformUI.getWorkbench()
												.getActiveWorkbenchWindow()
												.getShell(),
										GnuManagedMakeMessages.AltivecWarningDialogTitle,
										NLS
												.bind(
														GnuManagedMakeMessages.AltivecWarningDialogMessage,
														otherOption
																.getCommand(),
														option.getCommand()));
					}

 ...
 * 
 */
public class IncludePathsValueHandler extends PDTAffectedOptionValueHandler {

	/**
	 * 
	 */
	public IncludePathsValueHandler() {
		// Make default constructor available for extension point.
		super();
	}

	/**
	 * Handles the selection of include paths on the compiler.
	 * 
	 * Depending on the include paths included or excluded from the include
	 * paths list the PDT compiler shortcut option can be affected.
	 * 
	 ...
	 * the symbol definition option id, as this last option in conjunction with
	 * the include paths option defines if the compiler build is enabled for PDT
	 * use or not.
	 * 
	 * @param configuration
		...
		try {
			if (event == EVENT_APPLY) {
				setValue(configuration, holder, holder
						...
						arePDTCompilerBuildOptionsEnabled(holder, holder
								.getOptionBySuperClassId(arguments[1]), option));
			}
			return true;

...
/**
 * Displays SPU ProxyDMA.
 *
 * @author Ricardo M. Matinata
 * @since 1.3
 */
public class SPUProxyDMAView extends AbstractDebugEventHandlerView 
	...
	public class SPUProxyDMAViewLabelProvider extends LabelProvider implements ITableLabelProvider {
		...
		public String getColumnText( Object element, int columnIndex ) {
			if ( element instanceof MISPUProxyDMAElement ) {
				
				switch( columnIndex ) {
					case 0:
						return ((MISPUProxyDMAElement)element).getName();
					case 1:
						return ((MISPUProxyDMAElement)element).getValue();
	...
	protected Viewer createViewer( Composite parent ) {
		...
		SPUProxyDMAViewer vv = createMainViewer(parent);
		SPUProxyDMATupleViewer vv2 = createDetailsViewer();
	
		vv.setContentProvider( createContentProvider(vv2) );
		vv.setLabelProvider( new SPUProxyDMAViewLabelProvider() );
		vv.setUseHashlookup( true );
		
		vv2.setContentProvider(new SPUProxyDMATupleViewerContentProvider());
		vv2.setLabelProvider( new SPUProxyDMAViewLabelProvider() );
		
		CDebugUIPlugin.getDefault().getPreferenceStore().addPropertyChangeListener( this );

		// listen to selection in debug view
		getSite().getPage().addSelectionListener( IDebugUIConstants.ID_DEBUG_VIEW, this );
		setEventHandler( new SPUProxyDMAViewEventHandler( this ) );

		return vv;
	}
	
	protected SPUProxyDMAViewer createMainViewer(Composite parent) {
		...
		SPUProxyDMAViewer vv = new SPUProxyDMAViewer( composite, SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL | SWT.CENTER | SWT.BORDER);
		item.setControl(composite);
		item.setText(SPUProxyDMAMessages.getString("SPUProxyDMAView.0")); //$NON-NLS-1$
		
		return vv;
	}
	
	protected SPUProxyDMATupleViewer createDetailsViewer() {
		TabItem item = new TabItem(fSashform, SWT.NONE);
		Composite composite = new Composite(fSashform, SWT.NONE);
	    composite.setLayout(new FillLayout());
		SPUProxyDMATupleViewer vv2 = new SPUProxyDMATupleViewer( composite, SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL | SWT.CENTER | SWT.BORDER);
		Control control = vv2.getControl();
		GridData gd = new GridData( GridData.FILL_BOTH );
		control.setLayoutData( gd );
		item.setControl(composite);
		item.setText(SPUProxyDMAMessages.getString("SPUProxyDMAView.1")); //$NON-NLS-1$
	...
	private IContentProvider createContentProvider(Viewer attached) {
		SPUProxyDMAViewContentProvider cp = new SPUProxyDMAViewContentProvider(attached);

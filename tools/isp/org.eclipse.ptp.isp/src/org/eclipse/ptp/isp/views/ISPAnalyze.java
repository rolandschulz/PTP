...
import org.eclipse.ptp.isp.util.Envelope;
import org.eclipse.ptp.isp.util.InternalIssueOrderSorter;
import org.eclipse.ptp.isp.util.IspUtilities;
import org.eclipse.ptp.isp.util.ListElement;
import org.eclipse.ptp.isp.util.ProgramOrderSorter;
import org.eclipse.ptp.isp.util.Transitions;
...
/**
 * The ISP Analyzer View.
 */
public class ISPAnalyze extends ViewPart {

	// The ID for this view
	public static final String ID = "org.eclipse.ptp.isp.views.ISPAnalyze"; //$NON-NLS-1$
	 ...
	 * 
	 * @param logFilePath
	 *            The absolute path to the log file to parse.
	 * @return void
	 */
	 ...
	 * 
	 * @param logfile
	 *            A String containing the absolute path and name of the log
	 *            file.
	 * @return void
	 */
	...
	private void setMessageLabelText() {
		...
		// Interleaving label
		this.interleavingsGroup.setText(Messages.ISPAnalyze_10
	...
	private void launchCallBrowser() {
		...
		shell.setText(Messages.ISPAnalyze_17);
		shell.setImage(ISPPlugin.getImage(ISPPlugin
		...
		for (int i = 0; i < end; i++) {
			...
			if (currRank > prevRank) {
				...
				rankItem.setText(Messages.ISPAnalyze_20 + currRank);
				prevRank = currRank;
			}
	...
	private void launchLeakBrowser() {
		...
		Display display = wb.getDisplay();
		Shell shell = new Shell();
		shell.setText(Messages.ISPAnalyze_22);
		shell.setImage(ISPPlugin.getImage(ISPPlugin
				.getImageDescriptor("icons/magnified-trident.gif"))); //$NON-NLS-1$
		shell.setLayout(new GridLayout());
		...
		for (int i = 0; i < listSize; i++) {
			...
			leakItem.setText(Messages.ISPAnalyze_24 + i + ": " //$NON-NLS-1$
					+ env.getLeakResource());
		...
		SelectionListener listener = new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (e.item.toString().contains(Messages.ISPAnalyze_26)) {
	...
	private void launchErrorBrowser() {
		...
		Shell shell = new Shell();
		this.errShell = shell;
		shell.setText(Messages.ISPAnalyze_28);
		shell.setImage(ISPPlugin.getImage(ISPPlugin
				.getImageDescriptor("icons/magnified-trident.gif"))); //$NON-NLS-1$
		shell.setLayout(new GridLayout());
		...
		SelectionListener listener = new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if (e.item.toString().contains(Messages.ISPAnalyze_31)) {
					String lineString = e.item.toString().substring(
							e.item.toString().lastIndexOf(" ")); //$NON-NLS-1$
					lineString = lineString.substring(1,
							lineString.length() - 1);
	...
	private String getCallText(Envelope env, boolean isLeft) {
		...
		if (env.isCollective() && this.lockedRank == -1) {
			...
			if (ranksCommunicatedTo != 0
				...
				ranks = Messages.ISPAnalyze_41 + totalSkipped[0]
						/ ranksCommunicatedTo + Messages.ISPAnalyze_42
	...
	private void reset() {
		...
		if (this.browseLeaksButton.isEnabled()) {
			this.browseLeaksButton.setImage(ISPPlugin.getImage(ISPPlugin
					.getImageDescriptor("icons/browse.gif"))); //$NON-NLS-1$
			this.browseLeaksButton.setText(Messages.ISPAnalyze_53);
		} else {
			this.browseLeaksButton.setImage(ISPPlugin.getImage(ISPPlugin
		...
		this.getHelp = new Action() {
			public void run() {
				PlatformUI.getWorkbench().getHelpSystem().displayHelpResource(
						"/org.eclipse.ptp.isp.help/html/analyzerView.html"); //$NON-NLS-1$
			}
		};
		this.getHelp.setText(""); //$NON-NLS-1$
		this.getHelp.setToolTipText(Messages.ISPAnalyze_57);
		this.getHelp.setImageDescriptor(ISPPlugin
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				ISPAnalyze.this.fillContextMenu(manager);
			}
		});
	...
	private void fillContextMenu(IMenuManager manager) {
		manager.add(this.getHelp);
		this.getHelp.setText(Messages.ISPAnalyze_58);
	...
	private void createTransitionsGroup(Composite parent) {
		...
		this.firstTransitionButton.setToolTipText(Messages.ISPAnalyze_61);
		...
		this.previousTransitionButton.setToolTipText(Messages.ISPAnalyze_62);
		this.previousTransitionButton.setEnabled(false);
		FormData tprevFormData = new FormData();
		tprevFormData.left = new FormAttachment(this.firstTransitionButton, 5);
		...
		this.nextTransitionButton.setToolTipText(Messages.ISPAnalyze_63);
		...
		this.lastTransitionButton.setToolTipText(Messages.ISPAnalyze_64);
		this.lastTransitionButton.setEnabled(false);
		FormData tlastFormData = new FormData();
		tlastFormData.left = new FormAttachment(this.nextTransitionButton, 5);
		...
		Font lockRanksComboFont = setFontSize(
				this.lockRanksComboList.getFont(), 9);
	...
	private void createInterleavingsGroup(Composite parent) {
		...
		Image nextItemImage = ISPPlugin.getImage(ISPPlugin
				.getImageDescriptor("icons/next-item.gif")); //$NON-NLS-1$
		Image deadlockImage = ISPPlugin.getImage(ISPPlugin
		...
		this.firstInterleavingButton.setToolTipText(Messages.ISPAnalyze_69);
		...
		iprevFormData.left = new FormAttachment(this.firstInterleavingButton, 5);
		...
		this.lastInterleavingButton.setToolTipText(Messages.ISPAnalyze_72);
		this.lastInterleavingButton.setEnabled(false);
		FormData ilastFormData = new FormData();
		ilastFormData.left = new FormAttachment(this.nextInterleavingButton, 5);
		...
		this.deadlockInterleavingButton.setToolTipText(Messages.ISPAnalyze_73);
	...
	private void createStepOrderGroup(Composite parent) {
		...
		Image endEarlyImage = ISPPlugin.getImage(ISPPlugin
		...
		String stepOrder = ISPPlugin.getDefault().getPreferenceStore()
				.getString(PreferenceConstants.ISP_PREF_STEP_ORDER);
		if (stepOrder.equals(Messages.ISPAnalyze_80)) {
		...
		Font buttonFont = setFontSize(this.programOrderButton.getFont(), 9);
	...
	private void createRuntimeGroup(Composite parent) {
		...
		Image uiImage = ISPPlugin.getImage(ISPPlugin
				.getImageDescriptor("icons/ui-trident.gif")); //$NON-NLS-1$

		// Runtime information group
		Group runtimeInfoGroup = new Group(parent, SWT.SHADOW_IN);
		runtimeInfoGroup.setText(Messages.ISPAnalyze_82);
		...
		deadlockMessageFormData.width = 240;
		...
		this.examineErrorsButton.setText(Messages.ISPAnalyze_83);
		this.examineErrorsButton.setToolTipText(Messages.ISPAnalyze_84);
		...
		this.browseLeaksButton.setText(Messages.ISPAnalyze_85);
		this.browseLeaksButton.setToolTipText(Messages.ISPAnalyze_86);
	...
	private void createCodeWindowsGroup(Composite parent) {
		// Get images for buttons from image cache
		Image sourceCallImage = ISPPlugin.getImage(ISPPlugin
		...
		this.leftCodeWindowExplenationLabel.setText(Messages.ISPAnalyze_93);
		this.rightCodeWindowExplenationLabel.setText(Messages.ISPAnalyze_94);
	...
	private void createSelectionListeners() {
				...
				if (lockRanksComboList.getText() == null) {
					...
					try {
						String temp = lockRanksComboList.getItems()[lockRanksComboList
								.getSelectionIndex()];
						if (!temp.equals(Messages.ISPAnalyze_97)) {
					...
					public void widgetSelected(SelectionEvent e) {
						if (transitions.hasPreviousInterleaving()) {
							...
							IspUtilities.showInformationDialog(
									Messages.ISPAnalyze_98,
									Messages.ISPAnalyze_99);
						}
						updateErrorButtonState();
					}
				});
				...
				.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						if (transitions.hasPreviousInterleaving()) {
							...
							IspUtilities.showInformationDialog(
									Messages.ISPAnalyze_100,
									Messages.ISPAnalyze_101);
						}
						errorIndex = 1;
						updateErrorButtonState();
					}
				...
				.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						if (transitions.hasNextInterleaving()) {
							...
							IspUtilities.showInformationDialog(
									Messages.ISPAnalyze_102,
									Messages.ISPAnalyze_103);
						}
						errorIndex = 1;
						updateErrorButtonState();
					}
		...
		this.setRankComboList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (setRankComboList.getText() == null) {
					...
					ISPPlugin.getDefault().getPreferenceStore().setValue(
							PreferenceConstants.ISP_PREF_NUMPROCS, newNumProcs);
				}
			}
		});
		...
		this.launchIspUIButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				String sourceFilePath = ISPPlugin.getDefault()
						.getPreferenceStore().getString(
								PreferenceConstants.ISP_PREF_LAST_FILE);
				IspUtilities.doJavaGUI(sourceFilePath);
			}
		});
		...
		this.browseCallsButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (logContents == null || logContents.isEmpty()) {
					String message = Messages.ISPAnalyze_106;
					String title = Messages.ISPAnalyze_107;
					IspUtilities.showErrorDialog(message, title);
					return;
				}
				launchCallBrowser();
			}
		...
		this.browseLeaksButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (logContents == null || logContents.isEmpty()) {
					String message = Messages.ISPAnalyze_108;
					String title = Messages.ISPAnalyze_109;
					IspUtilities.showErrorDialog(message, title);
					return;
				}
				launchLeakBrowser();
	...
	private void updateErrorButtonState() {
		...
		if (this.examineErrorsButton.isEnabled()) {
			...
			this.examineErrorsButton.setText(Messages.ISPAnalyze_110);
			if (this.transitions.hasDeadlock()) {
				this.examineErrorsButton
						.setToolTipText(Messages.ISPAnalyze_111);
			} else {
				this.examineErrorsButton
						.setToolTipText(Messages.ISPAnalyze_112);
			}
		} else {
			this.examineErrorsButton.setImage(ISPPlugin.getImage(ISPPlugin
					.getImageDescriptor("icons/no-error.gif"))); //$NON-NLS-1$
			this.examineErrorsButton.setText(Messages.ISPAnalyze_113);
		}
	}
	...
	private void goToFirstTransition(boolean update) {
		...
		if (env == null) {
			IspUtilities.showInformationDialog(Messages.ISPAnalyze_114,
					Messages.ISPAnalyze_115);
			return;
		}
	...
	private void goToLastTransition() {
		...
		if (env == null) {
			IspUtilities.showInformationDialog(Messages.ISPAnalyze_118,
					Messages.ISPAnalyze_119);
			return;
		}
		this.oldLeftIndex = env.getLinenumber();
		setButtonEnabledState();

...
public class StatusTools implements IRemoteStatusTools {
	...
	/**
	 * This class is responsible for caching information that is "stable" enough
	 * to be stored and retrieved instead of generated every time.
	 * 
	 * @author Richard Maciel
	 * 
	 */
	class UserInformation {
		...
		private final Set<Integer> groupIDSet;
	 ...
	 * 
	 * @see
	 * org.eclipse.ptp.remotetools.core.IRemoteStatusTools#getRemotePortsInUse
	 * (int)
	 */
	public Set<Integer> getRemotePortsInUse(int protocol) throws RemoteConnectionException, RemoteOperationException,
			CancelException {
		// Uses the following shell script to get a list of used ports in the
		// remote host:
		// cat /proc/net/<transport protocol>
		// Then, the data coming from the command is filtered using java
		// commands equivalent to the
		// following shell command:
		// cat /proc/net/tcp | tail -n +2 | sed -r 's/^\s+//g' | cut -d ' ' -f 2
		// | cut -d ':' -f 2
		...
		try {
			...
			/*
			 * "protoOutput" and "proto6Output" are the outputs of the above
			 * commands, WITHOUT the header that is being wiped out by the
			 * "replaceFirst" method. Thus, the resulting string "rawProcOutput"
			 * contains only valid connection entries for both ipv4 and ipv6 and
			 * must be totally processed on the following loop.
			 */
	...
	/**
	 * An implementation of the {@link getGroupIDSet} method that only uses the
	 * cat command on the remote machine to extract desired data. Specifically,
	 * it retrieves information from the /etc/passwd looking and /etc/group
	 * files and extract the information about the groups of the user whose
	 * username comes from the {@link getUsername} method
	 * 
	 * @return
	...
	private Set<Integer> getGroupIDSet_CatBased() throws RemoteConnectionException, RemoteOperationException, CancelException {
		...
		try {
			...
			for (int i = 0; i < rawGroupLines.length; i++) {
				// Last field can be non-existant, so use a negative parameter
				// in split to
				...
				// IF the username belongs to the group, insert it into the
				// list.
	...
	/**
	 * An implementation of the {@link getUserID} method that only uses the cat
	 * command on the remote machine to extract desired data. Specifically it
	 * reads information from the /etc/passwd file and parses it to retrieve the
	 * user id.
	 * 
	 * @return
	 * @throws RemoteConnectionException
	 ...
	 * 
	 * @return String [] A vector containing the fields or null if no user
	 *         matches
	 * @throws RemoteOperationException
	 * @throws CancelException
	 * @throws RemoteConnectionException
	...
	private String[] getPasswdFields() throws RemoteOperationException, RemoteConnectionException, CancelException {
		...
		try {
			...
			// Look for the string that matches the value from the
			// currentUsername
	...
	/**
	 * * An implementation of the {@link getUsername} method that uses the
	 * whoami command on the remote machine to extract username information.
	 * 
	 * @return
	 * @throws RemoteConnectionException
	 * @throws RemoteOperationException
	...
	/**
	 * A method designed to parse the /etc/group file to find the list of groups
	 * that a user is enrolled in.
	 * 
	 * @param username
	 *            The name of the user that we are searching for enrollments.
	...
	private Set<Integer> readGroupList(String username) throws RemoteConnectionException, RemoteOperationException {
		...
		try {
			...
			// This case is going to happen if the format is not the expected
			// one.
	...
	/**
	 * Return all the user information from the remote machine. This operation
	 * is a bit slow and should be used only once per RemoteStatusTools object.
	 * 
	 * @throws RemoteConnectionException
	 * @throws RemoteOperationException
	 * @throws CancelException
		...
		try {
			...
			if (userInfoFields.length == 3) {
				...
				if (userInfoFields[2].trim().equalsIgnoreCase("")) { //$NON-NLS-1$
					groupList = readGroupList(userName);
				} else {
				...
				try {
					uid = new Integer(userInfoFields[1]);
				} catch (NumberFormatException e) {
				}
			}
			return new UserInformation(uid, groupList, userName);
		} catch (RemoteExecutionException e) {
			throw new RemoteOperationException(e);
		}
	}

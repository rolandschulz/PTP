...
public class Loops implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		while(selit.hasNext())
		{
			...
			//String fileroot = cbit.getParent().getParent().getResource().getLocation().toOSString();
			//String fileslash="*"+File.separator;
//System.out.println(location+" has a "+fileroot);
			...
			{
				//System.out.println("Path 'o function's file "+cbit.getResource().getFullPath());
				...
				try {
					...
					//selrouts.add("loops file=\""+fileslash+cbit.getUnderlyingResource().getName()+"\" routine=\""+returntype+" "+signature+"#"+"\"");
					//if(useclean)
					selrouts.add("loops file=\""+cbit.getUnderlyingResource().getName()+"\" routine=\""+returntype+" "+signature+"#"+"\"");
			...
			{
				//selfiles.add("loops file=\""+fileslash+cbit.getElementName()+"\" routine=\"#\"");
				//if(useclean)
				selfiles.add("loops file=\""+cbit.getElementName()+"\" routine=\"#\"");
				tot++;
			}
			else
			if(type==-1)
			{
				...
				{
					//selrouts.add("loops file=\""+fileslash+cbit.getUnderlyingResource().getName()+"\" routine=\""+cbit.getElementName().toUpperCase()+"#"+"\"");
					//if(useclean)
					selrouts.add("loops file=\""+cbit.getUnderlyingResource().getName()+"\" routine=\""+"#"+cbit.getElementName().toUpperCase()+"\"");
				}
				tot++;
			}
		...
		if(tot>0)
		{
			...
			Selector selectinst = new Selector(cbit.getCProject().getResource().getLocation().toOSString());
			if(selfiles.size()>0)
				selectinst.addInst(selfiles);
			if(selrouts.size()>0)
				selectinst.addInst(selrouts);
		}
		/*

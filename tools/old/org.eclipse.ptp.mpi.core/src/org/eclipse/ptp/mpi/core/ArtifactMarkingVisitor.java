...
public class ArtifactMarkingVisitor implements IResourceDeltaVisitor, IResourceVisitor
{
    ...
    public boolean visitFile(IResource resource, Artifact[] artifacts)
    {
        try {
            ...
            removeMarkers(resource, this.markerID);
    ...
    protected void createArtifactMarker(IResource resource, Artifact artifact, String fn) throws CoreException
    {
        ...
        MarkerUtilities.createMarker(resource, attrs, this.markerID);
        if (traceOn)
            System.out.println("marker created: for " + fn + " - " + artifact.getShortName() + " line "
                    + artifact.getLine() + " start " + artifact.getSourceInfo_().getStart() + " end "
                    + artifact.getSourceInfo_().getEnd());
    }
    ...
    protected Map createCommonMarkerAttrs(IResource resource, Artifact artifact, String fn)
    {
        ...
        attrs.put(MpiIDs.FILENAME, fn); // fn being processed
        attrs.put(MpiIDs.NAME, artifact.getShortName());
        attrs.put(MpiIDs.DESCRIPTION, artifact.getDescription());
        ...
        // attrs.put(MpiIDs.CONSTRUCT_TYPE, artifact.getSourceInfo_().getConstructType());

        // trying setting the marker to more precise location
        attrs.put(IMarker.CHAR_START, new Integer(artifact.getSourceInfo_().getStart()));
        attrs.put(IMarker.CHAR_END, new Integer(artifact.getSourceInfo_().getEnd()));

        // adding construct type, i.e. func call, constants, ect.
        attrs.put(MpiIDs.CONSTRUCT_TYPE, new Integer(artifact.getSourceInfo_().getConstructType()));

        String id = ((Artifact) artifact).getUniqueID();
        attrs.put(MpiIDs.ID, id); // used to look up to get artifact from marker later
        // for remed.
        attrs.put(MpiIDs.LINE, Integer.toString(artifact.getLine()));
        if (traceOn)

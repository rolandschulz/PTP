...
public class TableView extends ViewPart {
	...
	private final class LMLTableListListener implements ILMLListener {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.IMarkObjectEvent)
		 */
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.ISelectObjectEvent)
		 */
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.ITableSortedEvent)
		 */
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.IUnmarkObjectEvent)
		 */
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.IUnselectedObjectEvent)
		 */
		 ...
		 * 
		 * @see org.eclipse.ptp.rm.lml.core.listeners.ILMLListener# handleEvent
		 * (org.eclipse.ptp.core.events.IViewUpdateEvent)
		 */
	...
	private void createColumns() {
		...
		for (int i = 0; i < tableColumnLayouts.length; i++) {
			...
			if (tableColumnLayouts[i].isActive()) {
				 ...
				 */
				treeColumnLayout.setColumnData(
						treeColumn,
						new ColumnWeightData((int) (tableColumnLayouts[i]
								.getWidth() * composite.getClientArea().width),
								10, resizable));
			} else {
		...
		final Listener sortListener = new Listener() {
			public void handleEvent(Event e) {
				...
				if (fLguiItem != null && fLguiItem.getTableHandler() != null) {
					fLguiItem.getTableHandler().setSortProperties(gid,
							getSortIndex(), getSortDirectionString());
				}
				lmlManager.sortLgui();
			}
		};
	...
	private void createTable() {
		if (fLguiItem != null) {
			...
			if (fLguiItem.getTableHandler() != null) {
				final Object[] sortProperties = fLguiItem.getTableHandler()
						.getSortProperties(gid);

				if (!((String) sortProperties[1])
						.equals(ITableColumnLayout.SORT_DIRECTION_NONE)
	 ...
	 * Disposing the table.
	 * 
	 * Setting defaults values to sort column and sort direction. Deleting for
	 * every TreeColumn corresponding listeners and the TreeColumn itself. All
	 * MenuItems from the headerMenu and the MenuManager of the View are also
	 * erased.
	 */
	...
	/**
	 * Determines the sort direction and gives back the int value of the
	 * determined sort direction. Default value is "NONE".
	 * 
	...
	/**
	 * Determines the sort column and gives back the index of the determined
	 * sort column. Default value is -1.
	 * 
	...
	private Double[] getWidths() {
		...
		for (int i = 0; i < columns.length - 2; i++) {
			...
			if (column.getWidth() > 0) {
				columnWidth = Integer.valueOf(column.getWidth()).doubleValue()
						/ width;
			}
	 ...
	 * 
	 * The table of the TableView has two additional columns (one visible, the
	 * other is hidden) than there are saved in the inner model. This methods
	 * eliminates these two columns and rewrite the numbers of the column order.
	 * The visible one should always be the first, but during moving other
	 * columns this fact can change. The hidden column is always the last one.
	 * 
	 * @param order
	...
	/**
	 * Determines the current Layout of the view and transmit it to the inner
	 * model.
	 * 
	 * Sends first changes of the column order to the model side, then changes
	 * of the width. After that the sorting parameters are determined. There can
	 * be changes of the column order, so the position of the column is checked
	 * also.
	 * 

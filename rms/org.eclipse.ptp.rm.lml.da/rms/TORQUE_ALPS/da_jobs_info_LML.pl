...
foreach $jobid (sort(keys(%jobs))) {
	...
	$jobs{$jobid}{totaltasks} = $jobs{$jobid}{"Resource_List.size"}  if(exists($jobs{$jobid}{"Resource_List.size"}));
    }
...
#	$jobs{$jobid}{spec}       = $jobs{$jobid}{"Resource_List.size"}  if(exists($jobs{$jobid}{"Resource_List.size"}));
#    }    

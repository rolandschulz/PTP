...
Planned Changes to LML_da:
--------------------------

For integration of LML_da into the Eclipse/PTP framework the handling of the
input files has to be adapted. In special, LML_da will accept instead of a
LML_da config file also an LML request file which will be generated by the LML
view components of the PTP client. This request will be very similar to the
layout configuration file which describes which data is requested.  LML_da
will in this case automatic detect the type of batch system used on the remote
system and which LML_da configuration file should be used to generate the LML
data. This will be either done by scanning the request file (the type of batch
system could be obtained from the control part of the RM) or by searching for
the batch query command on the remote system (e.g. qstat or llq).

 ...
 *
 */
public class PBSServiceProviderFactory extends AbstractResourceManagerFactory {

	public static final String RM_FACTORY_ID = "org.eclipse.ptp.rm.pbs.resourcemanager"; //$NON-NLS-1$
	
	private static final String LAUNCH_SERVICE = "org.eclipse.ptp.core.LaunchService"; //$NON-NLS-1$
	private static final String PBS_SERVICE_PROVIDER = "org.eclipse.ptp.rm.pbs.PBSServiceProvider"; //$NON-NLS-1$
	...
	public IResourceManagerControl create(IResourceManagerConfiguration config) {
		...
		return new PBSResourceManager(Integer.valueOf(universe.getNextResourceManagerId()), universe, config);
	}
	...
	public IResourceManagerConfiguration createConfiguration() {
		IServiceConfiguration config = fServiceManager.newServiceConfiguration("PBS"); //$NON-NLS-1$
		IService service = fServiceManager.getService(LAUNCH_SERVICE);
		if (service != null) {
			IServiceProviderDescriptor desc = service.getProviderDescriptor(PBS_SERVICE_PROVIDER);
			if (desc != null) {

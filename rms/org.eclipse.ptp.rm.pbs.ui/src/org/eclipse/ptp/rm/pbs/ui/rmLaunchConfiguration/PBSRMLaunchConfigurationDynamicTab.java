...
public class PBSRMLaunchConfigurationDynamicTab extends
	...
	private static final String NUMBER_OF_PROCESSES_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_0;
	//jobNumNodes
	/*
	...
	*/
	...
	private static final String JOB_TIME_LIMIT_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_1;
	//jobPartition
	private String partationRequestedString = "3"; //$NON-NLS-1$
	private static final String JOB_PARTATION_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_2;
	//jobType
	private String jobTypeComboString = "mpi"; //$NON-NLS-1$
	private static final String JOB_TYPE_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_3;
	//jobIoLabel
	private String jobIoString = Messages.PBSRMLaunchConfigurationDynamicTab_4;
	private static final String JOB_IO_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_5;
	//jobVerbose
	private String jobVerboseModString = Messages.PBSRMLaunchConfigurationDynamicTab_6;
	private static final String JOB_VERBOSE_MODE_LABEL = Messages.PBSRMLaunchConfigurationDynamicTab_7;
	//jobNodeList
	private String jobNodeListString = "node0,node1"; //$NON-NLS-1$
	private static final String JOB_NODE_LIST_LABEL =Messages.PBSRMLaunchConfigurationDynamicTab_8;
	...
	public void createControl(Composite parent,	IResourceManager rm, IPQueue queue) {
		...
		numProcsText.setToolTipText(Messages.PBSRMLaunchConfigurationDynamicTab_9);
        ...
        */
		...
		TimeLimitText.setToolTipText(Messages.PBSRMLaunchConfigurationDynamicTab_10);
	...
	public RMLaunchValidation isValid(ILaunchConfiguration configuration,
		...
		try {
			...
			IntegerAttribute iattr = new IntegerAttribute(numProcsAttrDef, numProcsString);
			if (iattr.getValue() < 1) {
				return new RMLaunchValidation(false, Messages.PBSConfigurationWizardPage_numProcsInvalid);
			}
		} catch (IllegalValueException e) {
			return new RMLaunchValidation(false, NUMBER_OF_PROCESSES_LABEL + e.getMessage());
		}
		...
		try {
			...
			IntegerAttribute iattr = new IntegerAttribute(jobNumNodesAttrDef, numbernodesString);
			if (iattr.getValue() < 1) {
				return new RMLaunchValidation(false, Messages.getString("PBSConfigurationWizardPage.numbernodesInvalid")); //$NON-NLS-1$
			}
		} catch (IllegalValueException e) {
			return new RMLaunchValidation(false, JOB_NUMBER_OF_NODES_LABEL + e.getMessage());
		}
		*/
		try {
			...
			IntegerAttribute iattr = new IntegerAttribute(jobTimeLimitAttrDef, timeLimitString);
			if (iattr.getValue() < 1) {
				return new RMLaunchValidation(false, Messages.PBSConfigurationWizardPage_timeLimitInvalid);
			}
		} catch (IllegalValueException e) {
			return new RMLaunchValidation(false, JOB_TIME_LIMIT_LABEL + e.getMessage());
		}
		...
		try {
			...
			IntegerAttribute iattr = new IntegerAttribute(jobPartationAttrDef, partationRequestedString);
			if (iattr.getValue() < 1) {
				return new RMLaunchValidation(false, Messages.getString("PBSConfigurationWizardPage.numProcsInvalid")); //$NON-NLS-1$
			}
		} catch (IllegalValueException e) {
			return new RMLaunchValidation(false, JOB_PARTATION_LABEL + e.getMessage());
		}
	...
	private IntegerAttributeDefinition getJobNumNodesAttrDef(IResourceManager rm, IPQueue queue) {
		return PBSJobAttributes.getNumberOfNodesAttributeDefinition();
	}
	private IntegerAttributeDefinition getJobTimeLimitAttrDef(IResourceManager rm, IPQueue queue) {
		return PBSJobAttributes.getTimeLimitAttributeDefinition();
	}
	/*

 ...
 *
 */
public class IBMLLServiceProviderFactory extends AbstractResourceManagerFactory {

	public static final String RM_FACTORY_ID = "org.eclipse.ptp.rm.ibm.ll.resourcemanager"; //$NON-NLS-1$
	
	private static final String LAUNCH_SERVICE = "org.eclipse.ptp.core.LaunchService"; //$NON-NLS-1$
	private static final String PE_SERVICE_PROVIDER = "org.eclipse.ptp.rm.ibm.ll.LLServiceProvider"; //$NON-NLS-1$
	...
	public IResourceManagerControl create(IResourceManagerConfiguration config) {
		...
		return new IBMLLResourceManager(Integer.valueOf(universe.getNextResourceManagerId()), universe, config);
	}
	...
	public IResourceManagerConfiguration createConfiguration() {
		IServiceConfiguration config = fServiceManager.newServiceConfiguration("IBM LL"); //$NON-NLS-1$
		IService service = fServiceManager.getService(LAUNCH_SERVICE);
		if (service != null) {
			IServiceProviderDescriptor desc = service.getProviderDescriptor(PE_SERVICE_PROVIDER);
			if (desc != null) {

...
final public class ORTEResourceManagerConfiguration extends
	...
	public static IResourceManagerConfiguration load(
			ORTEResourceManagerFactory factory, IMemento memento) {
		...
		ORTEResourceManagerConfiguration config = new ORTEResourceManagerConfiguration(
	...
	public void setDefaultNameAndDesc() {
		String name = "ORTE"; //$NON-NLS-1$
		String conn = getConnectionName();
		if (conn != null && !conn.equals(EMPTY_STRING)) {
			name += "@" + conn; //$NON-NLS-1$
		}
		setName(name);
		setDescription("ORTE Resource Manager"); //$NON-NLS-1$
	}

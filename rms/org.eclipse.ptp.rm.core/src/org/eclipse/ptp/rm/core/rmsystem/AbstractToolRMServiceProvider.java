...
public abstract class AbstractToolRMServiceProvider extends AbstractRemoteResourceManagerServiceProvider implements IToolRMConfiguration {
	...
	public boolean hasContinuousMonitorCmd() {
		return (fCapabilities & CAP_CONTINUOUS_MONITOR) != 0
		&& getContinuousMonitorCmd() != null
	...
	public boolean hasDebugCmd() {
		return (fCapabilities & CAP_LAUNCH) != 0 && getDebugCmd() != null
		&& !getDebugCmd().trim().equals(EMPTY_STRING);
	}
	...
	public boolean hasDiscoverCmd() {
		return (fCapabilities & CAP_DISCOVER) != 0 && getDiscoverCmd() != null
		&& !getDiscoverCmd().trim().equals(EMPTY_STRING);
	}

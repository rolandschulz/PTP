...
public class LSFProxyRuntimeClient extends ProxyRuntimeClient implements IRuntimeProxy, IProxyRuntimeEventListener {
	...
	public boolean startup(final IProgressMonitor monitor) {
		System.out.println("LSFProxyRuntimeClient - firing up proxy, waiting for connecting.  Please wait!  This can take a minute . . .");
		
		System.out.println("LSF_SERVER path = '"+proxyPath+"'");
		...
		try {
			...
			if (launchManually) {
				if (monitor != null) {
					monitor.subTask("Waiting for manual lauch of ptp_lsf_proxy on port "+getSessionPort()+"...");
				}
			} else {
								...
								public void run() {
									try {
										...
										while ((output = out_reader.readLine()) != null) {
											System.out.println("++++++++++ ptp_lsf_proxy: " + output);
										}
									} catch (IOException e) {
										e.printStackTrace();
									}
	...
	public void shutdown() {
		try {
			System.out.println("LSFProxyRuntimeClient shutting down server...");
			setWaitEvent(IProxyRuntimeEvent.EVENT_RUNTIME_OK);
			sessionFinish();
			waitForRuntimeEvent();
			System.out.println("LSFProxyRuntimeClient shut down.");
		} catch (IOException e) {
			PTPCorePlugin.log(e);
		}
	}	
    ...
    public synchronized void handleEvent(IProxyRuntimeEvent e) {
		System.out.println("LSFProxyRuntimeClient got event: " + e.toString());
		
		if (waitEvents.get(e.getEventID())) {
			System.out.println("LSFProxyRuntimeClient notifying...");

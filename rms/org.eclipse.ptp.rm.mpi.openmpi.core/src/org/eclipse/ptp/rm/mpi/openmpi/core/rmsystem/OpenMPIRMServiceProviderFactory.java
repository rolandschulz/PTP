 ...
 *
 */
public class OpenMPIRMServiceProviderFactory extends AbstractResourceManagerFactory {

	public static final String RM_FACTORY_ID = "org.eclipse.ptp.rm.mpi.openmpi.resourcemanager"; //$NON-NLS-1$
	
	private static final String LAUNCH_SERVICE = "org.eclipse.ptp.core.LaunchService"; //$NON-NLS-1$
	private static final String OMPI_SERVICE_PROVIDER = "org.eclipse.ptp.rm.mpi.openmpi.OpenMPIServiceProvider"; //$NON-NLS-1$
	...
	public IResourceManagerControl create(IResourceManagerConfiguration config) {
		...
		return new OpenMPIResourceManager(Integer.valueOf(universe.getNextResourceManagerId()), universe, config);
	}
	...
	public IResourceManagerConfiguration createConfiguration() {
		IServiceConfiguration config = fServiceManager.newServiceConfiguration(Messages.OpenMPIResourceManagerConfiguration_defaultName);
		IService service = fServiceManager.getService(LAUNCH_SERVICE);
		if (service != null) {
			IServiceProviderDescriptor desc = service.getProviderDescriptor(OMPI_SERVICE_PROVIDER);
			if (desc != null) {

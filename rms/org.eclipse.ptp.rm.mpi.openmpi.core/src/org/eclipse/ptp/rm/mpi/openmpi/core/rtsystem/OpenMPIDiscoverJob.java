...
public class OpenMPIDiscoverJob extends AbstractRemoteCommandJob {
	...
	private OpenMPIHostMap readHostFile(IRemoteConnection connection, IRemoteServices remoteServices, IRemoteFileManager fileMgr,
		...
		DebugUtil.trace(DebugUtil.RTS_DISCOVER_TRACING,
				"rds_hostfile_path: {0}", (rds_param == null ? "null" : rds_param.getValue())); //$NON-NLS-1$  //$NON-NLS-2$
		DebugUtil.trace(DebugUtil.RTS_DISCOVER_TRACING,
				"orte_default_hostfile: {0}", (orte_param == null ? "null" : orte_param.getValue())); //$NON-NLS-1$  //$NON-NLS-2$
		...
		if (hostFilePath == null) {
			if (rmConfiguration.getDetectedVersion().equals(IOpenMPIResourceManagerConfiguration.VERSION_12)) {
				...
				throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID,
						Messages.OpenMPIDiscoverJob_Exception_DiscoverCommandMissingHostFilePath));
			}
		...
		if (!hostFilePath.isAbsolute()) {
			if (rmConfiguration.getDetectedVersion().equals(IOpenMPIResourceManagerConfiguration.VERSION_12)) {
				throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID, NLS.bind(
						Messages.OpenMPIDiscoverJob_Exception_DiscoverCommandHostFilePathNotAbsolute, hostFilePath)));
			}
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID, NLS.bind(
					Messages.OpenMPIDiscoverJob_Exception_DiscoverCommandFailedReadHostFile, hostFileName), e));
		}
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID, NLS.bind(
					Messages.OpenMPIDiscoverJob_Exception_DiscoverCommandFailedParseHostFile, hostFileName), e));
		}
		...
		if (hostMap.count() == 0) {
			if (rmConfiguration.getDetectedVersion().equals(IOpenMPIResourceManagerConfiguration.VERSION_12)) {
				// This was not correct for remote hosts. Worked only for local
				// hosts.
				// try {
				...
				// "Cannot retrive network information for local machine. Check network configuration."));
				// }
				String hostname = getRemoteHostname(connection, remoteServices);
				hostMap.addDefaultHost(hostname);
				DebugUtil.trace(DebugUtil.RTS_DISCOVER_TRACING,
						"Hostfile is empty. Added default host {0} for Open MPI 1.2.", hostname); //$NON-NLS-1$
			} else {
				DebugUtil.error(DebugUtil.RTS_DISCOVER_TRACING, "Empty hostfile is not allowed."); //$NON-NLS-1$
				throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID, NLS.bind(
						Messages.OpenMPIDiscoverJob_Exception_DiscoverCommandHostFileEmpty, hostFileName)));
			}
		}
		return hostMap;
	}

	private String getRemoteHostname(IRemoteConnection connection, IRemoteServices remoteServices) throws CoreException,
			IOException {
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, OpenMPIPlugin.PLUGIN_ID,
					Messages.OpenMPIDiscoverJob_Exception_HostnameCommandFailed, e));
		}
		...
		try {
			...
			if (DebugUtil.RTS_DISCOVER_TRACING) {
				...
				for (Parameters.Parameter param : info.getParameters()) {
					System.out.println(NLS.bind("  {0}={1}", param.getName(), param.getValue())); //$NON-NLS-1$
				}
			}
		} catch (IOException e) {

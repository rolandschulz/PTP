 ...
 *
 */
public class SLURMServiceProviderFactory extends AbstractResourceManagerFactory {

	public static final String RM_FACTORY_ID = "org.eclipse.ptp.rm.slurm.resourcemanager"; //$NON-NLS-1$
	
	private static final String LAUNCH_SERVICE = "org.eclipse.ptp.core.LaunchService"; //$NON-NLS-1$
	private static final String SLURM_SERVICE_PROVIDER = "org.eclipse.ptp.rm.slurm.SLURMServiceProvider"; //$NON-NLS-1$
	...
	public IResourceManagerControl create(IResourceManagerConfiguration config) {
		...
		return new SLURMResourceManager(Integer.valueOf(universe.getNextResourceManagerId()), universe, config);
	}
	...
	public IResourceManagerConfiguration createConfiguration() {
		IServiceConfiguration config = fServiceManager.newServiceConfiguration("SLURM"); //$NON-NLS-1$
		IService service = fServiceManager.getService(LAUNCH_SERVICE);
		if (service != null) {
			IServiceProviderDescriptor desc = service.getProviderDescriptor(SLURM_SERVICE_PROVIDER);
			if (desc != null) {

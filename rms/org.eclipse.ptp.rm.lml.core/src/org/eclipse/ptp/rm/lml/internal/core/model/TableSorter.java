...
public class TableSorter implements Comparator<RowType>{
	 ...
	 * Constructor
	 * 
	 * @param sortType type how the method should sort
	 * @param up comparison value (value for sorting upwards)
	 * @param sortIndex index of the to sorting column
	 * @param sortDirection value of the to sorting direction
	 */
	 ...
	 * If their are equal, the method returns 0. Returns -1, 0 or 1 as the first argument is less than, equal to, or greater than the second.
	...
	public int compare(RowType a, RowType b) {
		...
		if (sortType.equals("date")) {
			// The sort type is date
			Date aDate = null;
			Date bDate = null;
			try {
				aDate = LMLCorePlugin.getDefault().getSimpleDateFormat(a.getCell().get(sortIndex).getValue().length()).parse(a.getCell().get(sortIndex).getValue());
				bDate = LMLCorePlugin.getDefault().getSimpleDateFormat(b.getCell().get(sortIndex).getValue().length()).parse(b.getCell().get(sortIndex).getValue());
			} catch (ParseException e) {
				return 0;
			}

			if (sortDirection == up) {
				return aDate.compareTo(bDate) < 0 ? -1 : 1;
			} else {
				return aDate.compareTo(bDate) < 0 ? 1 : -1;
			}
			
		} else if (sortType.equals("numeric")) {
			// The sort type is a numeric one
			if (sortDirection == up){
				return Integer.parseInt(a.getCell().get(sortIndex).getValue()) < Integer.parseInt(b.getCell().get(sortIndex).getValue()) ? -1 : 1;
			} else {
				return Integer.parseInt(a.getCell().get(sortIndex).getValue()) < Integer.parseInt(b.getCell().get(sortIndex).getValue()) ? 1 : -1;
			}
		} else {
